...
function FunctionTypeAnnotation(node, parent, parentId, tokenContext) {
  return (
    ...
    parentId === 90 ||
    parentId === 4 ||
    (tokenContext & _index.TokenContext.arrowFlowReturnType) > 0
  );
}
...
function DoExpression(node, parent, parentId, tokenContext) {
  return (
    (tokenContext & _index.TokenContext.expressionStatement) > 0 && !node.async
  );
}
...
function BinaryExpression(node, parent, parentId, tokenContext) {
  ...
  return (
    (tokenContext & _index.TokenContext.forInOrInitHeadAccumulate) > 0 &&
    node.operator === "in"
  );
}
...
function SequenceExpression(node, parent, parentId) {
  if (
    parentId === 144 ||
    parentId === 133 ||
    (parentId === 108 && parent.property === node) ||
    (parentId === 132 && parent.property === node) ||
    parentId === 224
  ) {
    return false;
  }
...
function YieldExpression(node, parent, parentId) {
  return (
    ...
    parentId === 145 ||
    hasPostfixPart(node, parent, parentId) ||
    (parentId === 8 && isYieldExpression(node)) ||
    (parentId === 28 && node === parent.test) ||
    isClassExtendsClause(node, parent, parentId) ||
...
function UnaryLike(node, parent, parentId) {
  return (
    hasPostfixPart(node, parent, parentId) ||
    (parentId === 10 && parent.operator === "**" && parent.left === node) ||
    isClassExtendsClause(node, parent, parentId)
  );
}
...
function Identifier(node, parent, parentId, tokenContext, getRawIdentifier) {
  ...
  if (
    tokenContext & _index.TokenContext.forOfHead ||
    ((parentId === 108 || parentId === 132) &&
      tokenContext &
        (_index.TokenContext.expressionStatement |
