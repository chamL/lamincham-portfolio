...
exports.default = (0, util_1.createRule)({
  ...
  create(context) {
    ...
    const isLiteral = (0, util_1.isNodeOfType)(
    ...
    function isNaNIdentifier(node) {
      return (
        node.type === utils_1.AST_NODE_TYPES.Identifier && node.name === "NaN"
      );
    }
    function isFixableIdentifier(node) {
      return (
        (0, util_1.isUndefinedIdentifier)(node) ||
    ...
    function hasCommentsBetweenQuasi(startQuasi, endQuasi) {
      const startToken = (0, util_1.nullThrows)(
        ...
        util_1.NullThrowsReasons.MissingToken(
          "`${",
          "opening template literal",
        ),
      );
      const endToken = (0, util_1.nullThrows)(
        context.sourceCode.getTokenByRangeStart(endQuasi.range[0]),
        util_1.NullThrowsReasons.MissingToken("}", "closing template literal"),
      );
    ...
    function getLiteral(node) {
      const maybeLiteral =
        node.type === utils_1.AST_NODE_TYPES.TSLiteralType
          ? node.literal
          : node;
    ...
    function isUnncessaryTypeInterpolation({
      ...
      if (
        interpolation.type === utils_1.AST_NODE_TYPES.TSNullKeyword ||
    ...
    return {
      ...
      TSTemplateLiteralType(node) {
        if (
          ...
          const { constraintType, isTypeParameter } = (0,
          util_1.getConstraintInfo)(
            checker,
