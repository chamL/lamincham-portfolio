    ...
    return {
      ...(mode === "index-signature" && {
        TSTypeReference(node) {
          ...
          const shouldFix =
            indexParam.type === utils_1.AST_NODE_TYPES.TSStringKeyword ||
      ...
      ...(mode === "record" && {
        ...
        TSMappedType(node) {
          ...
          const scopeManagerKey = (0, util_1.nullThrows)(
          ...
          if (parentId) {
            const scope = context.sourceCode.getScope(key);
            const superVar = utils_1.ASTUtils.findVariable(
              scope,
              parentId.name,
            );
            if (superVar) {
...
function isDeeplyReferencingType(node, superVar, visited) {
  ...
  switch (node.type) {
    ...
    case utils_1.AST_NODE_TYPES.Identifier: {
      ...
      if (
        superVar.references.some((ref) =>
          (0, util_1.isNodeEqual)(ref.identifier, node),
        )
      ) {
        return true;
      }
