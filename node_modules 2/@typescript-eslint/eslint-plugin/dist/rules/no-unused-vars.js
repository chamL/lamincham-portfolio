...
(function (VariableType) {
  ...
  VariableType[(VariableType["ImplicitGlobalVariable"] = 5)] =
...
const isCommaToken = {
  predicate: (token) =>
    token.type === utils_1.AST_TOKEN_TYPES.Punctuator && token.value === ",",
  tokenChar: ",",
};
      ...
      switch (def.node.type) {
        ...
        case utils_1.AST_NODE_TYPES.ImportDefaultSpecifier: {
          ...
          return {
            ...
            fix: (fixer) => {
              const comma = (0, util_1.nullThrows)(
                context.sourceCode.getTokenAfter(def.node),
                util_1.NullThrowsReasons.MissingToken(",", "import specifier"),
              );
      ...
      for (const { used, variable } of variables) {
        ...
        if (def.type === utils_1.TSESLint.Scope.DefinitionType.ClassName) {
          const hasStaticBlock = def.node.body.body.some(
            (node) => node.type === utils_1.AST_NODE_TYPES.StaticBlock,
          );
        ...
        if (def.type === utils_1.TSESLint.Scope.DefinitionType.CatchClause) {
          ...
          def.type === utils_1.TSESLint.Scope.DefinitionType.Parameter
        ) {
        ...
        for (const unusedVar of unusedVars) {
          ...
          if (unusedVar.defs.length > 0) {
            const usedOnlyAsType = unusedVar.references.some((ref) =>
              (0, referenceContainsTypeQuery_1.referenceContainsTypeQuery)(
                ref.identifier,
              ),
            );
            ...
            const isImportUsedOnlyAsType =
              ...
              unusedVar.defs.some(
                (def) =>
                  def.type === scope_manager_1.DefinitionType.ImportBinding,
              );
