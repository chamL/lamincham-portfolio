...
function validateDefaultProjectForFilesGlob(allowDefaultProject) {
  ...
  for (const glob of allowDefaultProject) {
    if (glob === "*") {
      throw new Error(
        `allowDefaultProject contains the overly wide '*'.${exports.DEFAULT_PROJECT_FILES_ERROR_EXPLANATION}`,
      );
    }
    if (glob.includes("**")) {
      throw new Error(
        `allowDefaultProject glob '${glob}' contains a disallowed '**'.${exports.DEFAULT_PROJECT_FILES_ERROR_EXPLANATION}`,
      );
    }
  }
}
