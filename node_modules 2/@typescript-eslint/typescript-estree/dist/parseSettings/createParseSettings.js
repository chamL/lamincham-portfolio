...
function createParseSettings(code, tsestreeOptions = {}) {
  ...
  const tsconfigRootDir = (() => {
    if (tsestreeOptions.tsconfigRootDir == null) {
      const inferredTsconfigRootDir = (0,
      candidateTSConfigRootDirs_1.getInferredTSConfigRootDir)();
      if (
        node_path_1.default.resolve(inferredTsconfigRootDir) !==
        inferredTsconfigRootDir
      ) {
        throw new Error(
          `inferred tsconfigRootDir should be a resolved absolute path, but received: ${JSON.stringify(inferredTsconfigRootDir)}. This is a bug in typescript-eslint! Please report it to us at https://github.com/typescript-eslint/typescript-eslint/issues/new/choose.`,
        );
      }
    ...
    if (typeof tsestreeOptions.tsconfigRootDir === "string") {
      ...
      if (
        !node_path_1.default.isAbsolute(userProvidedTsconfigRootDir) ||
        // Ensure it's fully absolute with a drive letter if windows
        (process.platform === "win32" &&
          !/^[a-zA-Z]:/.test(userProvidedTsconfigRootDir))
      ) {
        throw new Error(
          `parserOptions.tsconfigRootDir must be an absolute path, but received: ${JSON.stringify(userProvidedTsconfigRootDir)}. This is a bug in your configuration; please supply an absolute path.`,
        );
      }
  ...
  const filePath = (0, shared_1.ensureAbsolutePath)(
