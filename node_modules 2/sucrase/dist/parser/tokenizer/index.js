...
function pushTypeContext(existingTokensInType) {
  for (
    let i = _base.state.tokens.length - existingTokensInType;
    i < _base.state.tokens.length;
    i++
  ) {
...
function readToken(code) {
  ...
  if (
    ...
    (code === _charcodes.charCodes.atSign &&
      _base.input.charCodeAt(_base.state.pos + 1) ===
        _charcodes.charCodes.atSign)
  ) {
...
function readToken_dot() {
  ...
  if (
    nextChar >= _charcodes.charCodes.digit0 &&
    nextChar <= _charcodes.charCodes.digit9
  ) {
    readNumber(true);
    return;
  }
  ...
  if (
    ...
    _base.input.charCodeAt(_base.state.pos + 2) === _charcodes.charCodes.dot
  ) {
    _base.state.pos += 3;
...
function readToken_mult_modulo(code) {
  // '%*'
  let tokenType =
    code === _charcodes.charCodes.asterisk
      ? _types.TokenType.star
      : _types.TokenType.modulo;
  let width = 1;
  ...
  if (
    code === _charcodes.charCodes.asterisk &&
    nextChar === _charcodes.charCodes.asterisk
  ) {
    width++;
  ...
  if (
    ...
    _base.input.charCodeAt(_base.state.pos + 2) !==
      _charcodes.charCodes.greaterThan
  ) {
    width++;
...
function readToken_pipe_amp(code) {
  ...
  if (nextChar === code) {
    if (
      ...
      // || or &&
      finishOp(
        code === _charcodes.charCodes.verticalBar
          ? _types.TokenType.logicalOR
          : _types.TokenType.logicalAND,
        2,
      );
    }
    return;
  ...
  if (code === _charcodes.charCodes.verticalBar) {
    ...
    if (nextChar === _charcodes.charCodes.greaterThan) {
      ...
      nextChar === _charcodes.charCodes.rightCurlyBrace &&
      _base.isFlowEnabled
    ) {
      // '|}'
  ...
  finishOp(
    code === _charcodes.charCodes.verticalBar
      ? _types.TokenType.bitwiseOR
      : _types.TokenType.bitwiseAND,
    1,
  );
}
  ...
  if (nextChar === _charcodes.charCodes.greaterThan) {
    const size =
      ...
      _charcodes.charCodes.greaterThan
        ...
        : 2;
    if (
      _base.input.charCodeAt(_base.state.pos + size) ===
      _charcodes.charCodes.equalsTo
    ) {
...
function readToken_eq_excl(code) {
  ...
  if (
    code === _charcodes.charCodes.equalsTo &&
    nextChar === _charcodes.charCodes.greaterThan
  ) {
    // '=>'
    _base.state.pos += 2;
...
function readToken_question() {
  ...
  if (
    ...
    nextChar === _charcodes.charCodes.dot &&
    !(
      nextChar2 >= _charcodes.charCodes.digit0 &&
      nextChar2 <= _charcodes.charCodes.digit9
    )
  ) {
  ...
  switch (code) {
    ...
    case _charcodes.charCodes.leftCurlyBrace:
      if (
        _base.isFlowEnabled &&
        _base.input.charCodeAt(_base.state.pos + 1) ===
          _charcodes.charCodes.verticalBar
      ) {
...
function readNumber(startsWithDot) {
  ...
  if (
    nextChar === _charcodes.charCodes.uppercaseE ||
    nextChar === _charcodes.charCodes.lowercaseE
  ) {
    nextChar = _base.input.charCodeAt(++_base.state.pos);
    if (
      nextChar === _charcodes.charCodes.plusSign ||
      nextChar === _charcodes.charCodes.dash
    ) {
      ++_base.state.pos;
    }
    readInt();
...
function readTmplToken() {
  for (;;) {
    ...
    if (
      ...
      (ch === _charcodes.charCodes.dollarSign &&
        _base.input.charCodeAt(_base.state.pos + 1) ===
          _charcodes.charCodes.leftCurlyBrace)
    ) {
      if (
        _base.state.pos === _base.state.start &&
        match(_types.TokenType.template)
      ) {
...
function skipWord() {
  while (_base.state.pos < _base.input.length) {
    ...
    if (_identifier.IS_IDENTIFIER_CHAR[ch]) {
      ...
      _base.state.pos += 2;
      if (
        _base.input.charCodeAt(_base.state.pos) ===
        _charcodes.charCodes.leftCurlyBrace
      ) {
        while (
          _base.state.pos < _base.input.length &&
          _base.input.charCodeAt(_base.state.pos) !==
            _charcodes.charCodes.rightCurlyBrace
        ) {
          _base.state.pos++;
        }
        _base.state.pos++;
