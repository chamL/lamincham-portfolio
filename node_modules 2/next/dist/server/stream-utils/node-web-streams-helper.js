...
function createInstantTestScriptInsertionTransformStream(requestId) {
  ...
  return new TransformStream({
    transform(chunk, controller) {
      ...
      const headOpenIndex = (0, _uint8arrayhelpers.indexOfUint8Array)(
        chunk,
        _encodedtags.ENCODED_TAGS.OPENING.HEAD,
      );
      if (headOpenIndex === -1) {
        controller.enqueue(chunk);
        return;
      ...
      const headCloseAngle = chunk.indexOf(
        62,
        headOpenIndex + _encodedtags.ENCODED_TAGS.OPENING.HEAD.length,
      );
      if (headCloseAngle === -1) {
        controller.enqueue(chunk);
        return;
  ...
  return new TransformStream({
    transform(chunk, controller) {
      ...
      if (index > -1) {
        ...
        if (
          chunk.length === _encodedtags.ENCODED_TAGS.CLOSED.BODY_AND_HTML.length
        ) {
          return;
        }
...
function createHtmlDataDplIdTransformStream(dplId) {
  ...
  return new TransformStream({
    transform(chunk, controller) {
      ...
      const htmlTagIndex = (0, _uint8arrayhelpers.indexOfUint8Array)(
        chunk,
      ...
      const insertionPoint =
        htmlTagIndex + _encodedtags.ENCODED_TAGS.OPENING.HTML.length;
