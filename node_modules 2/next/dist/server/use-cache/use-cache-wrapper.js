...
const findSourceMapURL =
  process.env.NODE_ENV !== "production"
    ? require("../lib/source-maps").findSourceMapURLDEV
    : undefined;
  ...
  const pendingCoarseEntry = savedCacheResult.then((collectedResult) => {
    ...
    if (rootParamNames && rootParamNames.size > 0 && rootParams) {
      ...
      const rootParamTags = [...rootParamNames].map(
        (paramName) => _constants1.NEXT_CACHE_ROOT_PARAM_TAG_ID + paramName,
      );
      return {
...
function createUseCacheStore(workStore, cacheContext, defaultCacheLife) {
  if (cacheContext.kind === "private") {
    ...
    return {
      ...
      phase: "render",
      implicitTags:
        outerWorkUnitStore == null ? void 0 : outerWorkUnitStore.implicitTags,
      ...
      tags: null,
      hmrRefreshHash: (0, _workunitasyncstorageexternal.getHmrRefreshHash)(
        outerWorkUnitStore,
      ),
      isHmrRefresh: (0, _workunitasyncstorageexternal.isHmrRefresh)(
        outerWorkUnitStore,
      ),
      serverComponentsHmrCache: (0,
      _workunitasyncstorageexternal.getServerComponentsHmrCache)(
        outerWorkUnitStore,
      ),
    ...
    return {
      ...
      isHmrRefresh:
        (useCacheOrRequestStore == null
          ? void 0
          : useCacheOrRequestStore.isHmrRefresh) ?? false,
...
function generateCacheEntryWithCacheContext(
  ...
  return _workunitasyncstorageexternal.workUnitAsyncStorage.run(
    cacheStore,
    () =>
      _dynamicaccessasyncstorageexternal.dynamicAccessAsyncStorage.run(
        {
...
async function collectResult(
  ...
  if (!cacheContext.skipPropagation) {
    ...
    const cacheSignal = (0, _workunitasyncstorageexternal.getCacheSignal)(
      cacheContext.outerWorkUnitStore,
    );
    if (cacheSignal) {
      cacheSignal.endRead();
...
async function generateCacheEntryImpl(
  ...
  const resultPromise = (0, _lazyresult.createLazyResult)(
    fn.bind(null, ...args),
  );
  let errors = [];
  ...
  const handleError = (0, _createerrorhandler.createReactServerErrorHandler)(
    process.env.NODE_ENV === "development",
  ...
  switch (outerWorkUnitStore.type) {
    ...
    case "prerender":
      ...
      const dynamicAccessAbortSignal =
        (_dynamicAccessAsyncStorage_getStore =
          _dynamicaccessasyncstorageexternal.dynamicAccessAsyncStorage.getStore()) ==
        null
          ? void 0
          : _dynamicAccessAsyncStorage_getStore.abortController.signal;
      const abortSignal = dynamicAccessAbortSignal
        ...
        : timeoutAbortController.signal;
      const { prelude } = await (0, _static.prerender)(
        resultPromise,
      ...
      if (timeoutAbortController.signal.aborted) {
        ...
        const hangingPromise = (0, _dynamicrenderingutils.makeHangingPromise)(
          outerWorkUnitStore.renderSignal,
          workStore.route,
          'dynamic "use cache"',
        );
    ...
    case "unstable-cache":
    case "generate-static-params":
      stream = (0, _server.renderToReadableStream)(
        resultPromise,
        clientReferenceManifest.clientModules,
        {
          environmentName: "Cache",
...
async function cache(kind, id, boundArgsLength, originalFn, args) {
  ...
  const cacheHandler = isPrivate
    ? undefined
    : (0, _handlers.getCacheHandler)(kind);
  if (!isPrivate && !cacheHandler) {
    throw Object.defineProperty(
      new Error("Unknown cache handler: " + kind),
      "__NEXT_ERROR_CODE",
      {
        value: "E248",
        enumerable: false,
  ...
  if (isPrivate) {
    ...
    switch (workUnitStore.type) {
      ...
      case "unstable-cache": {
        throw wrapAsInvalidDynamicUsageError(
          Object.defineProperty(
            new Error( // TODO: Add a link to an error documentation page when we have one.
      ...
      default:
        ...
        throw Object.defineProperty(
          new _invarianterror.InvariantError(`Unexpected work unit store.`),
          "__NEXT_ERROR_CODE",
          {
            value: "E737",
            enumerable: false,
  ...
  const hangingInputAbortSignal = (0,
  _dynamicrendering.createHangingInputAbortSignal)(workUnitStore);
  if (cacheContext.kind === "private") {
    ...
    switch (outerWorkUnitStore.type) {
      ...
      case "request": {
        if (process.env.NODE_ENV === "development") {
          ...
          await (0, _dynamicrenderingutils.makeDevtoolsIOAwarePromise)(
            undefined,
            outerWorkUnitStore,
            stage,
          );
  ...
  if (isPageSegmentFunction(args)) {
    ...
    fn = {
      [name]: async (
        { params: _innerParams, searchParams: innerSearchParams },
        ...otherInnerArgs
      ) =>
        originalFn.apply(null, [
          {
            params: outerParams,
            searchParams:
              innerSearchParams ?? // For public caches, search params are omitted from the cache
    ...
    fn = {
      [name]: async (
        { params: _innerParams, ...innerSlots },
        ...otherInnerArgs
      ) =>
        originalFn.apply(null, [
          {
  ...
  if (boundArgsLength > 0) {
    ...
    const boundArgs = await (0, _encryption.decryptActionBoundArgs)(
      id,
      encryptedBoundArgs,
    );
  ...
  const encodeCacheKeyParts = () =>
    (0, _client.encodeReply)(cacheKeyParts, {
      temporaryReferences,
    ...
    case "prerender":
      if (!isPageOrLayoutSegmentFunction) {
        ...
        encodedCacheKeyParts =
          await _dynamicaccessasyncstorageexternal.dynamicAccessAsyncStorage.run(
            {
  ...
  const serializedCacheKey =
    typeof encodedCacheKeyParts === "string" // Convert it to an ArrayBuffer if it wants to.
      ? encodedCacheKeyParts
  ...
  const implicitTags =
    ((_workUnitStore_implicitTags = workUnitStore.implicitTags) == null
      ? void 0
      : _workUnitStore_implicitTags.tags) ?? [];
    ...
    if (rdcEntry !== undefined) {
      ...
      if (rdcResult !== undefined) {
        if (
          ...
          rdcResult.entry.expire < _constants.DYNAMIC_EXPIRE
        ) {
          switch (workUnitStore.type) {
            case "prerender":
  ...
  if (stream === undefined) {
    ...
    if (
      lazyRefreshTags &&
      !(0, _lazyresult.isResolvedLazyResult)(lazyRefreshTags)
    ) {
      ...
      if (entry && rootParams) {
        ...
        for (const tag of entry.tags) {
          if (tag.startsWith(_constants1.NEXT_CACHE_ROOT_PARAM_TAG_ID)) {
            paramNames.add(
              tag.slice(_constants1.NEXT_CACHE_ROOT_PARAM_TAG_ID.length),
            );
          }
        }
        if (paramNames.size > 0) {
    ...
    if (entry) {
      ...
      if (workUnitStore.implicitTags) {
        ...
        if (lazyExpiration) {
          const expiration = (0, _lazyresult.isResolvedLazyResult)(
            lazyExpiration,
          )
    ...
    if (
      entry !== undefined &&
      (entry.revalidate === 0 || entry.expire < _constants.DYNAMIC_EXPIRE)
    ) {
      switch (workUnitStore.type) {
        case "prerender":
    ...
    if (
      ...
      if (currentTime > entry.timestamp + entry.revalidate * 1000) {
        ...
        // revalidated entry.
        const result = await generateCacheEntry(
          workStore, // The background revalidation preserves the outer store for reading
...
function isRecentlyRevalidatedTag(tag, workStore) {
  ...
  // store.
  if (
    pendingRevalidatedTags == null
      ? void 0
      : pendingRevalidatedTags.some((item) => item.tag === tag)
  ) {
