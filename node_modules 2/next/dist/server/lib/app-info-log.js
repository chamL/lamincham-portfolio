...
function logStartInfo({ networkUrl, appUrl, envInfo, logBundler }) {
  ...
  _log.bootstrap(
    `${(0, _picocolors.bold)((0, _picocolors.purple)(`${_log.prefixes.ready} Next.js ${"16.2.1"}`))}${versionSuffix}`,
  );
  if (appUrl) {
  ...
  if (envInfo == null ? void 0 : envInfo.length)
    _log.bootstrap(`- Environments: ${envInfo.join(", ")}`);
}
  ...
  if (experimentalFeatures == null ? void 0 : experimentalFeatures.length) {
    ...
    for (const exp of experimentalFeatures) {
      const isValid = Object.prototype.hasOwnProperty.call(
        _configschema.experimentalSchema,
        exp.key,
      );
      if (isValid) {
        const symbol =
        ...
        _log.bootstrap(
          `  ? ${(0, _picocolors.strikethrough)(exp.key)} (invalid experimental key)`,
        );
      }
    }
  }
