...
const _watchpack = /*#__PURE__*/ _interop_require_default(
    ...
    try {
      ...
      handleError(
        Object.defineProperty(
          new Error("Unexpected error during process lookup", {
            cause,
          }),
          "__NEXT_ERROR_CODE",
          {
...
async function startServer(serverOptions) {
  ...
  let requestHandler = async (req, res) => {
    ...
    throw Object.defineProperty(
      new Error("Invariant request handler was not setup"),
      "__NEXT_ERROR_CODE",
      {
        value: "E287",
        enumerable: false,
  ...
  let upgradeHandler = async (req, socket, head) => {
    ...
    throw Object.defineProperty(
      new Error("Invariant upgrade handler was not setup"),
      "__NEXT_ERROR_CODE",
      {
        value: "E290",
        enumerable: false,
  ...
  async function requestListener(req, res) {
    try {
      ...
      if (isDev) {
        if (
          _v8.default.getHeapStatistics().used_heap_size >
          0.8 * _v8.default.getHeapStatistics().heap_size_limit
        ) {
          _log.warn(
          ...
          (0, _trace.trace)(
            ...
            undefined,
            {
              "memory.heapSizeLimit": String(
                _v8.default.getHeapStatistics().heap_size_limit,
              ),
  ...
  const server = selfSignedCertificate
    ...
    : _http.default.createServer(requestListener);
  if (keepAliveTimeout) {
  ...
  let cleanupListeners = isDev
    ? new _asynccallbackset.AsyncCallbackSet()
    : undefined;
  const distDir = await new Promise((resolve) => {
    server.on("listening", async () => {
      const addr = server.address();
      const actualHostname = (0, _formathostname.formatHostname)(
        typeof addr === "object"
          ? (addr == null ? void 0 : addr.address) || hostname || "localhost"
          : addr,
      );
      const formattedHostname =
        !hostname || actualHostname === "0.0.0.0"
          ...
          : actualHostname === "[::]"
            ? "[::1]"
            : (0, _formathostname.formatHostname)(hostname);
      port =
        typeof addr === "object"
          ? (addr == null ? void 0 : addr.port) || port
          : port;
      if (portRetryCount) {
      ...
      const networkHostname =
        hostname ??
        (0, _getnetworkhost.getNetworkHost)(
          (0, _isipv6.isIPv6)(actualHostname) ? "IPv6" : "IPv4",
        );
      const protocol = selfSignedCertificate ? "https" : "http";
      const networkUrl = networkHostname
        ? `${protocol}://${(0, _formathostname.formatHostname)(networkHostname)}:${port}`
        : null;
      ...
      const formattedStartDuration = (0, _durationtostring.durationToString)(
  ...
  if (isDev) {
    function watchConfigFiles(dirToWatch, onChange) {
      ...
      wp.watch({
        files: _constants.CONFIG_FILES.map((file) =>
          _path.default.join(dirToWatch, file),
        ),
      });
