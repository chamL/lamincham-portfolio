...
async function isTrueCasePagePath(pagePath, pagesDir) {
  const pageSegments = (0, _path.normalize)(pagePath)
    .split(_path.sep)
    .filter(Boolean);
  const segmentExistsPromises = pageSegments.map(async (segment, i) => {
    const segmentParentDir = (0, _path.join)(
      pagesDir,
      ...pageSegments.slice(0, i),
    );
...
async function findPageFile(
  ...
  const pagePaths = (0, _getpagepaths.getPagePaths)(
    normalizedPagePath,
    pageExtensions,
    isAppDir,
  );
      ...
      pagePaths.map(async (path) => {
        ...
        try {
          ...
          if (
            !(err == null
              ? void 0
              : (_err_code = err.code) == null
                ? void 0
                : _err_code.includes("ENOTDIR"))
          )
            throw err;
        }
        return null;
  ...
  if (others.length > 0) {
    (0, _log.warn)(
      `Duplicate page detected. ${(0, _picocolors.cyan)((0, _path.join)("pages", existingPath))} and ${(0, _picocolors.cyan)((0, _path.join)("pages", others[0]))} both resolve to ${(0, _picocolors.cyan)(normalizedPagePath)}.`,
    );
  }
  return existingPath;
}
function createValidFileMatcher(pageExtensions, appDirPath) {
  ...
  const createRootOnlyPattern = (fileName) =>
   ...
   */ function isMetadataFile(filePath) {
    const appDirRelativePath = appDirPath
      ...
      : filePath;
    return (0, _ismetadataroute.isMetadataRouteFile)(
      appDirRelativePath,
      pageExtensions,
      true,
    );
