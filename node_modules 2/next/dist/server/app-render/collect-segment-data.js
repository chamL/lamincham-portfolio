...
async function collectSegmentData(
  ...
  const segmentTasks = [];
  const { prelude: treeStream } = await (0, _static.prerender)(
...
async function collectPrefetchHints(
  ...
  const initialRSCPayload = await (0, _client.createFromReadableStream)(
...
async function collectPrefetchHintsImpl(
  ...
  staleTime,
  seedData,
  clientModules, // TODO: Consider persisting the computed requestKey into the hints output
  ...
  for (const parallelRouteKey in children) {
    ...
    _segmentvalueencoding.appendSegmentRequestKeyPart)(
      requestKey,
      parallelRouteKey,
      (0, _segmentvalueencoding.createSegmentRequestKeyPart)(childSegment),
    );
    const childResult = await collectPrefetchHintsImpl(
      ...
      childSeedData,
      clientModules,
      childRequestKey, // Once a child has accepted us, stop offering to remaining siblings.
...
async function renderSegmentPrefetch(
  ...
  const { prelude: segmentStream } = await (0, _static.prerender)(
    segmentPrefetch,
    clientModules,
    {
      filterStackFrame,
