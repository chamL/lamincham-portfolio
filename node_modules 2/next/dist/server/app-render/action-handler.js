 ...
 */ function hasServerActions() {
  const serverActionsManifest = (0,
  _manifestssingleton.getServerActionsManifest)();
  return (
...
function getForwardedHeaders(req, res) {
  ...
  const requestCookies = new _cookies.RequestCookies(
    _headers.HeadersAdapter.from(requestHeaders),
  );
  // Get response headers and cookies
  const responseHeaders = res.getHeaders();
  const responseCookies = new _cookies.ResponseCookies(
    (0, _utils1.fromNodeOutgoingHttpHeaders)(responseHeaders),
  );
...
function addRevalidationHeader(res, { workStore, requestStore }) {
  ...
  const isTagRevalidated = (
    (_workStore_pendingRevalidatedTags = workStore.pendingRevalidatedTags) ==
    null
      ? void 0
    ...
    ? 1
    : 0;
  const isCookieRevalidated = (0, _requestcookies.getModifiedCookieValues)(
  ...
  if (isTagRevalidated || isCookieRevalidated) {
    res.setHeader(
      _approuterheaders.NEXT_ACTION_REVALIDATED_HEADER,
      JSON.stringify(
 ...
 */ async function createForwardedActionResponse(
  ...
  const proto =
    ((_getRequestMeta = (0, _requestmeta.getRequestMeta)(
      ...
      ? void 0
      : _getRequestMeta.replace(/:+$/, "")) || "https";
  // For standalone or the serverful mode, use the internal origin directly
  // other than the host headers from the request.
  const origin =
    process.env.__NEXT_PRIVATE_ORIGIN || `${proto}://${host.value}`;
  ...
  try {
    ...
    if (
      ...
      (0, _helpers.isWebNextRequest)(req)
    ) {
      if (!req.body) {
        throw Object.defineProperty(
          new Error("Invariant: missing request body."),
          "__NEXT_ERROR_CODE",
          {
            value: "E333",
            enumerable: false,
      ...
      throw Object.defineProperty(
        new Error("Invariant: Unknown request type."),
        "__NEXT_ERROR_CODE",
        {
          value: "E114",
          enumerable: false,
    ...
    if (
      (_response_headers_get = response.headers.get("content-type")) == null
        ? void 0
        : _response_headers_get.startsWith(
      ...
      var // Since we aren't consuming the response body, we cancel it to avoid memory leaks
        _response_body;
      (_response_body = response.body) == null
        ? void 0
        : _response_body.cancel();
    }
  } catch (err) {
  ...
  return _renderresult.default.fromStatic(
    "{}",
    _constants.JSON_CONTENT_TYPE_HEADER,
  );
}
/**
...
async function createRedirectRenderResult(
  ...
  if (appRelativeRedirectUrl) {
    ...
    if (workStore.pendingRevalidatedTags) {
      var _workStore_incrementalCache_prerenderManifest_preview,
      ...
      forwardedHeaders.set(
        ...
        ((_workStore_incrementalCache = workStore.incrementalCache) == null
          ? void 0
          : (_workStore_incrementalCache_prerenderManifest =
                _workStore_incrementalCache.prerenderManifest) == null
            ? void 0
            : (_workStore_incrementalCache_prerenderManifest_preview =
                  _workStore_incrementalCache_prerenderManifest.preview) == null
              ? void 0
              : _workStore_incrementalCache_prerenderManifest_preview.previewModeId) ||
          "",
      );
    }
    ...
    try {
      ...
      (0, _setcachebustingsearchparam.setCacheBustingSearchParam)(fetchUrl, {
        ...
        [_approuterheaders.NEXT_URL]:
          forwardedHeaders.get(_approuterheaders.NEXT_URL) ?? undefined,
      });
      const response = await fetch(fetchUrl, {
        method: "GET",
        headers: forwardedHeaders,
  ...
  const forwardedHostHeaderValue =
    forwardedHostHeader && Array.isArray(forwardedHostHeader)
      ...
      : forwardedHostHeader == null
        ? void 0
        : (_forwardedHostHeader_split = forwardedHostHeader.split(",")) == null
          ? void 0
          : (_forwardedHostHeader_split_ = _forwardedHostHeader_split[0]) ==
              null
            ? void 0
            : _forwardedHostHeader_split_.trim();
...
async function handleAction({
  ...
  } = (0, _serveractionrequestmeta.getServerActionRequestMetadata)(req);
  ...
  const originHost =
    typeof originHeader === "string" // However, these contexts can still send along credentials like cookies,
  ...
  if (!originHost) {
    ...
    if (
      (0, _csrfprotection.isCsrfOriginAllowed)(
        originHost,
        serverActions == null ? void 0 : serverActions.allowedOrigins,
      )
    ) {
      // Ignore it
    } else {
      if (host) {
        ...
        console.error(
          `\`${host.type}\` header with value \`${limitUntrustedHeaderValueForLogs(host.value)}\` does not match \`origin\` header with value \`${limitUntrustedHeaderValueForLogs(originHost)}\` from a forwarded Server Actions request. Aborting the action.`,
        );
      } else {
  ...
  if (actionId) {
    const forwardedWorker = (0, _manifestssingleton.selectWorkerForForwarding)(
      actionId,
      page,
    );
    ...
    return await actionAsyncStorage.run(
      ...
      async () => {
        ...
        if (
          ...
          const actionBodyFromMeta = (0, _requestmeta.getRequestMeta)(
            req,
            "actionBody",
          );
          const body = actionBodyFromMeta
            ? Readable.from(actionBodyFromMeta)
            : req.body;
          const defaultBodySizeLimit = "1 MB";
          const bodySizeLimit =
            (serverActions == null ? void 0 : serverActions.bodySizeLimit) ??
            defaultBodySizeLimit;
          const bodySizeLimitBytes =
            bodySizeLimit !== defaultBodySizeLimit
              ? require("next/dist/compiled/bytes").parse(bodySizeLimit)
              : 1024 * 1024; // 1 MB
          let size = 0;
        ...
        const actionHandler = actionMod[actionId]; // `actionId` must exist if we got here, as otherwise we would have thrown an error above
        // Log server action call in development when enabled
        let logInfo = null;
        const { type: actionType } = (0,
        _serverreferenceinfo.extractInfoFromServerReferenceId)(actionId);
        if (
          process.env.NODE_ENV === "development" &&
          ctx.renderOpts.logServerFunctions && // TODO: For now, skip logging for 'use cache' Server Functions as the
          ...
          const actionInfo =
            (_serverActionsManifest_runtime = serverActionsManifest[runtime]) ==
            null
              ? void 0
              : _serverActionsManifest_runtime[actionId];
          if (actionInfo) {
            var _actionInfo_exportedName;
            const isInlineAction =
              (_actionInfo_exportedName = actionInfo.exportedName) == null
                ? void 0
                : _actionInfo_exportedName.startsWith(INLINE_ACTION_PREFIX);
            const projectDir =
              ctx.renderOpts.dir ||
              (process.env.NEXT_RUNTIME === "edge" ? "" : process.cwd());
            const location = (0, _segmentexplorerpath.normalizeFilePath)(
              projectDir,
              actionInfo.filename,
            );
        ...
        if (isFetchAction) {
          ...
          const maybeRevalidatesPromise = skipPageRendering
            ? (0, _revalidationutils.executeRevalidates)(workStore)
            : false;
          return {
            type: "done",
            result: await generateFlight(req, ctx, requestStore, {
 ...
 */ function getActionModIdOrError(actionId, serverModuleMap) {
  ...
  if (!actionId) {
    throw Object.defineProperty(
      new _invarianterror.InvariantError("Missing 'next-action' header."),
      "__NEXT_ERROR_CODE",
      {
        value: "E664",
        enumerable: false,
  ...
  const actionModId =
    (_serverModuleMap_actionId = serverModuleMap[actionId]) == null
      ? void 0
      : _serverModuleMap_actionId.id;
  if (!actionModId) {
