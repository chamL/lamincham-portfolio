...
function getFlightStream(flightStream, debugStream, debugEndTime, nonce) {
  ...
  const { moduleLoading, edgeSSRModuleMapping, ssrModuleMapping } = (0,
  ...
  if (flightStream instanceof ReadableStream) {
    ...
    if (debugStream && !(debugStream instanceof ReadableStream)) {
      throw Object.defineProperty(
        new _invarianterror.InvariantError(
          "Expected debug stream to be a ReadableStream",
        ),
        "__NEXT_ERROR_CODE",
        {
          value: "E939",
    ...
    } = require("react-server-dom-webpack/client"); // eslint-disable-next-line import/no-extraneous-dependencies
    ...
    if (process.env.NEXT_RUNTIME === "edge") {
      ...
      if (debugStream && !(debugStream instanceof Readable)) {
        throw Object.defineProperty(
          new _invarianterror.InvariantError(
            "Expected debug stream to be a Readable",
          ),
          "__NEXT_ERROR_CODE",
          {
            value: "E940",
...
function writeInitialInstructions(controller, scriptStart, formState) {
  let scriptContents = `(self.__next_f=self.__next_f||[]).push(${(0,
  _htmlescape.htmlEscapeJsonString)(
    JSON.stringify([INLINE_FLIGHT_PAYLOAD_BOOTSTRAP]),
  )})`;
  if (formState != null) {
    scriptContents += `;self.__next_f.push(${(0,
    _htmlescape.htmlEscapeJsonString)(
      JSON.stringify([INLINE_FLIGHT_PAYLOAD_FORM_STATE, formState]),
    )})`;
  }
  controller.enqueue(
    encoder.encode(`${scriptStart}${scriptContents}</script>`),
  );
}
function writeFlightDataInstruction(controller, scriptStart, chunk) {
  let htmlInlinedData;
  if (typeof chunk === "string") {
    htmlInlinedData = (0, _htmlescape.htmlEscapeJsonString)(
