...
function createComponentTree(props) {
  return (0, _tracer.getTracer)().trace(
    _constants.NextNodeServerSpan.createComponentTree,
    {
...
async function createComponentTreeInternal(
  ...
  const { page, conventionPath, segment, modules, parallelRoutes } = (0,
  ...
  _tracer.getTracer)().trace(
    _constants.NextNodeServerSpan.getLayoutOrPageModule,
    {
  ...
  const instantConfig = layoutOrPageMod
    ? layoutOrPageMod.unstable_instant
    : undefined;
  ...
  if (
    typeof (layoutOrPageMod == null ? void 0 : layoutOrPageMod.fetchCache) ===
    "string"
  ) {
    workStore.fetchCache =
      layoutOrPageMod == null ? void 0 : layoutOrPageMod.fetchCache;
  }
  if (
    typeof (layoutOrPageMod == null ? void 0 : layoutOrPageMod.revalidate) !==
    "undefined"
  ) {
    (0, _patchfetch.validateRevalidate)(
      layoutOrPageMod == null ? void 0 : layoutOrPageMod.revalidate,
      workStore.route,
    );
  }
  if (
    ...
    if (
      ...
      defaultRevalidate === 0 && // If the postpone API isn't available, we can't postpone the render and
      // therefore we can't use the dynamic API.
      ...
      throw Object.defineProperty(
        new DynamicServerError(dynamicUsageDescription),
        "__NEXT_ERROR_CODE",
        {
          value: "E1005",
          enumerable: false,
  ...
  if (
    isPage &&
    typeof (layoutOrPageMod == null
      ? void 0
      : layoutOrPageMod.unstable_dynamicStaleTime) === "number"
  ) {
  ...
  const LayoutOrPage = layoutOrPageMod
    ? (0, _interopdefault.interopDefault)(layoutOrPageMod)
    : undefined;
  /**
  ...
  const optionalCatchAllParamName =
    (segmentParam == null ? void 0 : segmentParam.type) === "oc" &&
  ...
  const parallelRouteMap = await Promise.all(
    Object.keys(parallelRoutes).map(async (parallelRouteKey) => {
      ...
      if (
        ...
        isPrefetch &&
        (Loading ||
          !(0, _hasloadingcomponentintree.hasLoadingComponentInTree)(
            parallelRoute,
        ...
        if (process.env.NODE_ENV === "development" && missingSlots) {
          ...
          const parsedTree = (0, _parseloadertree.parseLoaderTree)(
            parallelRoute,
          );
          if (
            (_parsedTree_conventionPath = parsedTree.conventionPath) == null
              ? void 0
              : _parsedTree_conventionPath.endsWith(
      ...
      _segmentexplorerpath.getConventionPathByType)(tree, dir, "template");
      const errorFilePath = (0, _segmentexplorerpath.getConventionPathByType)(
        tree,
        dir,
        "error",
      );
      const loadingFilePath = (0, _segmentexplorerpath.getConventionPathByType)(
        tree,
        dir,
        "loading",
      );
      const globalErrorFilePath = isRoot
        ? (0, _segmentexplorerpath.getConventionPathByType)(
            tree,
            dir,
            "global-error",
          )
        ...
        ? createElement(
            ...
            globalErrorFilePath &&
              createElement(SegmentViewNode, {
                ...
                pagePath: (0, _segmentexplorerpath.isNextjsBuiltinFilePath)(
                  globalErrorFilePath,
                )
                  ? `${_segmentexplorerpath.BUILTIN_PREFIX}global-error.js${fileNameSuffix}`
                  : globalErrorFilePath,
              }),
          )
        : null;
  ...
  if (!MaybeComponent) {
    return createSeedData(
      ...
      isRuntimePrefetchable, // No user-provided component, so no params will be accessed. Use the
  ...
  if (
    ...
    return createSeedData(
      ...
      loadingData,
      true,
      isRuntimePrefetchable, // force-dynamic postpones without rendering the component, so no params
      // are accessed. The vary params are empty.
      _varyparams.emptyVaryParamsAccumulator,
    );
  }
  const isClientComponent = (0, _clientandserverreferences.isClientReference)(
    layoutOrPageMod,
  );
  const varyParamsAccumulator =
    isClientComponent && cacheComponents // from the server, so they have an empty vary params set.
      ? _varyparams.emptyVaryParamsAccumulator
      : (0, _varyparams.createVaryParamsAccumulator)();
  if (
  ...
  if (isPage) {
    ...
    const pageFilePath =
      (0, _segmentexplorerpath.getConventionPathByType)(tree, dir, "page") ??
      (0, _segmentexplorerpath.getConventionPathByType)(
        tree,
        dir,
        "defaultPage",
    ...
    if (isClientComponent) {
      ...
      let serverSegment;
      if (
        (0, _clientandserverreferences.isUseCacheFunction)(SegmentComponent)
      ) {
    ...
    const layoutFilePath = (0, _segmentexplorerpath.getConventionPathByType)(
      tree,
      dir,
      "layout",
    );
...
function getRootParamsImpl(
  ...
  const {
    modules: { layout },
    parallelRoutes,
  } = (0, _parseloadertree.parseLoaderTree)(loaderTree);
  ...
  if (isRootLayout) {
    ...
    return getRootParamsImpl(
      currentParams, // We stop looking for root params as soon as we hit the first layout
...
async function createBoundaryConventionElement({
  ...
  const element = Component
    ...
    : undefined;
  const pagePath = (0, _segmentexplorerpath.getConventionPathByType)(
    tree,
    dir,
    conventionName,
  );
  const wrappedElement =
    isSegmentViewEnabled && element
      ...
      : element;
  return [wrappedElement, pagePath];
}
function createSeedData(
      ...
      switch (workUnitStore.type) {
        ...
        case "prerender-runtime":
          ...
          if (stagedRendering) {
            ...
            rsc = stagedRendering
              .waitForStage(_stagedrendering.RenderStage.Static)
              .then(() => deferredRsc);
          }
          break;
        case "prerender":
  ...
  return [
    ...
    isPossiblyPartialResponse,
    varyParamsAccumulator
      ? (0, _varyparams.getVaryParamsThenable)(varyParamsAccumulator)
      : null,
  ];
}
