...
function createInlinedDataStream(source, nonce, formState) {
  return (0, _useflightresponse.createInlinedDataReadableStream)(
    source,
    nonce,
    formState,
  );
...
async function resumeToFizzStream(element, postponedState, streamOptions) {
  const stream = await (0, _server.resume)(
    element,
    postponedState,
    streamOptions,
  );
...
function pipeRuntimePrefetchTransform(stream, sentinel, isPartial, staleTime) {
  return stream.pipeThrough(
    (0, _nodewebstreamshelper.createRuntimePrefetchTransformStream)(
      sentinel,
      isPartial,
      staleTime,
    ),
