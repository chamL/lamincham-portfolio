...
function getMutableCookies(headers, onUpdateCookies) {
  const cookies = new _cookies.RequestCookies(
    _headers.HeadersAdapter.from(headers),
  );
  return _requestcookies.MutableRequestCookiesAdapter.wrap(
    cookies,
    onUpdateCookies,
  );
}
  ...
  return {
    ...
    get cookies() {
      if (!cache.cookies) {
        ...
        const requestCookies = new _cookies.RequestCookies(
          _headers.HeadersAdapter.from(req.headers),
        );
        mergeMiddlewareCookies(req, requestCookies);
        // Seal the cookies object that'll freeze out any methods that could
        // mutate the underlying data.
        cache.cookies =
          _requestcookies.RequestCookiesAdapter.seal(requestCookies);
      }
    ...
    get userspaceMutableCookies() {
      if (!cache.userspaceMutableCookies) {
        ...
        _requestcookies.createCookiesWithMutableAccessCheck)(this);
    ...
    get draftMode() {
      if (!cache.draftMode) {
        cache.draftMode = new _draftmodeprovider.DraftModeProvider(
          previewProps,
          req,
          this.cookies,
          this.mutableCookies,
...
function synchronizeMutableCookies(store) {
  ...
  store.cookies = _requestcookies.RequestCookiesAdapter.seal(
    (0, _requestcookies.responseCookiesToRequestCookies)(store.mutableCookies),
  );
}
