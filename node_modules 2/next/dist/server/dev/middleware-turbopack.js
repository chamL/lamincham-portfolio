...
function shouldIgnorePath(modulePath) {
  return (
    modulePath.includes("node_modules") || // Only relevant for when Next.js is symlinked e.g. in the Next.js monorepo
 ...
 */ async function batchedTraceSource(project, frame) {
  ...
  const currentDirectoryFileUrl = (0, _nodeurl.pathToFileURL)(
    process.cwd(),
  ).href;
 ...
 */ async function nativeTraceSource(frame) {
  ...
  try {
    var _findSourceMap;
    sourceMapPayload =
      (_findSourceMap = (0, _nodemodule.findSourceMap)(sourceURL)) == null
        ? void 0
  ...
  if (sourceMapPayload !== undefined) {
    ...
    if (traced !== null) {
      ...
      const applicableSourceMap = (0,
      _sourcemaps.findApplicableSourceMapPayload)(
        (frame.line ?? 1) - 1,
        (frame.column ?? 1) - 1,
        sourceMapPayload,
      );
      ...
      if (applicableSourceMap === undefined) {
        ...
        ignored =
          ((_applicableSourceMap_ignoreList = applicableSourceMap.ignoreList) ==
          null
            ? void 0
            : _applicableSourceMap_ignoreList.includes(sourceIndex)) ?? // When sourcemap is not available, fallback to checking `frame.file`.
...
async function createOriginalStackFrame(project, projectPath, frame) {
  const traced =
    (await nativeTraceSource(frame)) ?? // TODO(veil): When would the bundler know more than native?
  ...
  if (
    ...
    normalizedStackFrameLocation = _path.default.relative(
      projectPath,
      (0, _nodeurl.fileURLToPath)(normalizedStackFrameLocation),
    );
  }
...
function getSourceMapMiddleware(project) {
  return async function (req, res, next) {
    ...
    try {
      ...
      if (sourceMapString) {
        return _middlewareresponse.middlewareResponse.jsonString(
          res,
          sourceMapString,
        );
      }
