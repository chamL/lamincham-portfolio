...
function treePathToEntrypoint(segmentPath, parentPath) {
  ...
  const childSegmentPath = (0, _flightdatahelpers.getNextFlightSegmentPath)(
    segmentPath,
  );
  ...
  switch (dynamicParamTypeShort) {
    ...
    default:
      throw Object.defineProperty(
        new Error("Unknown dynamic param type"),
        "__NEXT_ERROR_CODE",
        {
          value: "E378",
          enumerable: false,
...
function getPageBundleType(pageBundlePath) {
  ...
  if ((0, _utils.isMiddlewareFilename)(pageBundlePath))
    return _pagetypes.PAGE_TYPES.ROOT;
...
class Invalidator {
  ...
  invalidate(compilerKeys = COMPILER_KEYS) {
    for (const key of compilerKeys) {
      ...
      (_this_multiCompiler_compilers_COMPILER_INDEXES_key_watching =
        this.multiCompiler.compilers[_constants.COMPILER_INDEXES[key]]
          .watching) == null
        ? void 0
        : _this_multiCompiler_compilers_COMPILER_INDEXES_key_watching.invalidate();
    }
  }
...
function disposeInactiveEntries(entries, maxInactiveAge) {
  Object.keys(entries).forEach((entryKey) => {
    ...
    if (
      (0, _utils.isMiddlewareFilename)(bundlePath) ||
      (0, _utils.isInstrumentationHookFilename)(bundlePath)
    ) {
      return;
    }
    if (dispose)
...
async function findPagePathData(
  ...
  let pagePath = null;
  const isInstrumentation = (0, _utils.isInstrumentationHookFile)(
    normalizedPagePath,
  );
  if ((0, _utils.isMiddlewareFile)(normalizedPagePath) || isInstrumentation) {
    pagePath = await (0, _findpagefile.findPageFile)(
      rootDir,
      normalizedPagePath,
      extensions,
      false,
    ...
    const pageUrl = (0, _ensureleadingslash.ensureLeadingSlash)(
      (0, _removepagepathtail.removePagePathTail)(
        (0, _normalizepathsep.normalizePathSep)(pagePath),
        {
          extensions,
        },
      ),
  ...
  if (appDir) {
    if (page === _constants.UNDERSCORE_NOT_FOUND_ROUTE_ENTRY) {
      ...
      if (isGlobalNotFoundEnabled) {
        const globalNotFoundPath = await (0, _findpagefile.findPageFile)(
          appDir,
          "global-not-found",
          extensions,
          true,
        ...
        const notFoundPath = await (0, _findpagefile.findPageFile)(
          appDir,
          "not-found",
          extensions,
          true,
  ...
  if (pagePath !== null && pagesDir) {
    ...
    return {
      ...
      bundlePath: _path.posix.join(
        "pages",
        (0, _normalizepagepath.normalizePagePath)(pageUrl),
      ),
      page: _path.posix.normalize(pageUrl),
    };
  }
...
function onDemandEntryHandler({
  ...
  function getPagePathsFromEntrypoints(type, entrypoints) {
    ...
    for (const entrypoint of entrypoints.values()) {
      const page = (0, _getroutefromentrypoint.default)(
        entrypoint.name,
        hasAppDir,
      );
      if (page) {
        var _entrypoint_name;
        const pageBundleType = (
          (_entrypoint_name = entrypoint.name) == null
            ? void 0
            : _entrypoint_name.startsWith("app/")
        )
          ? _pagetypes.PAGE_TYPES.APP
          : _pagetypes.PAGE_TYPES.PAGES;
        pagePaths.push(getEntryKey(type, pageBundleType, page));
      } else if (
        (0, _utils.isMiddlewareFilename)(entrypoint.name) ||
        (0, _utils.isInstrumentationHookFilename)(entrypoint.name)
      ) {
        pagePaths.push(
          getEntryKey(type, _pagetypes.PAGE_TYPES.ROOT, `/${entrypoint.name}`),
        );
      }
    }
    return pagePaths;
  ...
  for (const compiler of multiCompiler.compilers) {
    compiler.hooks.done.tap("NextJsOnDemandEntries", () => {
      var _getInvalidator;
      return (_getInvalidator = getInvalidator(compiler.outputPath)) == null
        ? void 0
        : _getInvalidator.doneBuilding([compiler.name]);
    });
  }
  multiCompiler.hooks.done.tap("NextJsOnDemandEntries", (multiStats) => {
    ...
    (_getInvalidator = getInvalidator(multiCompiler.outputPath)) == null
      ? void 0
      : _getInvalidator.doneBuilding([...COMPILER_KEYS]);
  ...
  async function ensurePageImpl({ page, appPaths, definition, isApp, url }) {
    ...
    try {
      ...
      if (typeof isApp === "boolean" && isApp !== isInsideAppDir) {
        ...
        throw Object.defineProperty(
          new Error(
            `Ensure bailed, found path "${route.page}" does not match ensure type (${isApp ? "app" : "pages"})`,
          ),
          "__NEXT_ERROR_CODE",
          {
            value: "E419",
      ...
      const addEntry = (compilerType) => {
        ...
        if (
          curEntries[entryKey] && // there can be an overlap in the entryKey for the instrumentation hook file and a page named the same
          ...
          !(0, _utils.isInstrumentationHookFilename)(
            curEntries[entryKey].bundlePath,
          )
        ) {
      ...
      const isServerComponent =
        isInsideAppDir && staticInfo.rsc !== _constants.RSC_MODULE_TYPES.client;
      ...
      (0, _entries.runDependingOnPageType)({
        ...
        onClient: () => {
          ...
          added.set(
            _constants.COMPILER_NAMES.client,
            addEntry(_constants.COMPILER_NAMES.client),
          );
        },
        onServer: () => {
          added.set(
            _constants.COMPILER_NAMES.server,
            addEntry(_constants.COMPILER_NAMES.server),
          );
          ...
          if (
            ...
            !(0, _utils.isInstrumentationHookFile)(route.page)
          ) {
      ...
      if (hasNewEntry) {
        const routePage = isApp
          ? route.page
          : (0, _apppaths.normalizeAppPath)(route.page);
        // If proxy file, remove the leading slash from "/proxy" to "proxy".
        (0, _output.reportTrigger)(
          (0, _utils.isMiddlewareFile)(routePage)
            ? routePage.slice(1)
            : routePage,
          url,
        );
    ...
    onHMR(client, getHmrServerError) {
      ...
      client.addEventListener("message", ({ data }) => {
        try {
          ...
          if (
            parsedData.event ===
            _hotreloadertypes.HMR_MESSAGE_SENT_TO_SERVER.PING
          ) {
