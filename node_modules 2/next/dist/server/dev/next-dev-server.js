...
const ReactDevOverlay = (props) => {
  if (PagesDevOverlayBridgeImpl === undefined) {
    PagesDevOverlayBridgeImpl =
      require("../../next-devtools/userspace/pages/pages-dev-overlay-setup").PagesDevOverlayBridge;
  }
...
class DevServer extends _nextserver.default {
  ...
  constructor(options) {
    ...
    if (this.nextConfig.experimental.serverComponentsHmrCache) {
      ...
      const hmrCacheSize = Math.max(
        ...
        _configshared.defaultConfig.cacheMaxMemorySize,
      );
    ...
    return (req, res, parsedUrl) => {
      ...
      if (loggingConfig !== false) {
        ...
        const isMiddlewareRequest =
          (0, _requestmeta.getRequestMeta)(req, "middlewareInvoke") ?? false;
        if (!isMiddlewareRequest) {
          response.originalResponse.once("close", () => {
            ...
            // The closure variable is not used here because the request handler may be invoked twice for one request when middleware is added in the application.
            // By setting the start time we can ensure that the middleware timing is correctly included.
            const requestStart = (0, _requestmeta.getRequestMeta)(
              req,
              "devRequestTimingStart",
            );
            if (!requestStart) {
            ...
            const devRequestTimingInternalsEnd = (0,
            _requestmeta.getRequestMeta)(req, "devRequestTimingInternalsEnd");
            if (devRequestTimingInternalsEnd) {
              this.startServerSpan.manualTraceChild(
                "render-path",
                (0, _trace.hrtimeToEpochNanoseconds)(
                  devRequestTimingInternalsEnd,
                ),
                (0, _trace.hrtimeToEpochNanoseconds)(requestEnd),
                {
                  path: req.url || "",
                },
              );
  ...
  async loadInstrumentationModule() {
    ...
    if (
      ...
      try {
        instrumentationModule = await (0,
        _instrumentationglobalsexternal.getInstrumentationModule)(
          this.dir,
  ...
  async runInstrumentationHookIfAvailable() {
    await (0, _instrumentationglobalsexternal.ensureInstrumentationRegistered)(
      this.dir,
      this.nextConfig.distDir,
    );
  }
    ...
    const nextInvoke = (0, _coalescedfunction.withCoalescedInvoke)(
      ...
      .then(async (res) => {
        ...
        if (isAppPath) {
          ...
          if (
            isAppPath &&
            this.nextConfig.cacheComponents && // Ensure this is not the first invocation.
            result &&
            ((_result_prerenderedRoutes = result.prerenderedRoutes) == null
              ? void 0
              : _result_prerenderedRoutes.length) !==
              (prerenderedRoutes == null ? void 0 : prerenderedRoutes.length)
          ) {
        ...
        const value = {
          staticPaths:
            prerenderedRoutes == null
              ? void 0
              : prerenderedRoutes.map((route) => route.pathname),
          prerenderedRoutes,
          fallbackMode: fallback,
        };
        if (
          ((_res_value = res.value) == null
            ? void 0
            : _res_value.fallbackMode) !== undefined && // This matches the hasGenerateStaticParams logic we do during build.
          ...
          const rawExistingManifest = await _fs.default.promises.readFile(
            (0, _path.join)(this.distDir, _constants1.PRERENDER_MANIFEST),
            "utf8",
          );
          ...
          const fallbackPrerenderedRoute =
            prerenderedRoutes == null
              ? void 0
          ...
          existingManifest.dynamicRoutes[pathname] = {
            dataRoute: null,
            dataRouteRegex: null,
            fallback: (0, _fallback.fallbackModeToFallbackField)(
            ...
            fallbackRootParams:
              fallbackPrerenderedRoute == null
                ? void 0
                : fallbackPrerenderedRoute.fallbackRootParams,
            fallbackRouteParams:
              fallbackPrerenderedRoute == null
                ? void 0
