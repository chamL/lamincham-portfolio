...
const _serveractionlogger = require("./server-action-logger");
function ignoreLoggingIncomingRequests(request, loggingConfig) {
  ...
  if (
    typeof (loggingConfig == null ? void 0 : loggingConfig.incomingRequests) ===
    "boolean"
  ) {
  ...
  const ignore =
    loggingConfig == null
      ? void 0
      : (_loggingConfig_incomingRequests = loggingConfig.incomingRequests) ==
          null
        ? void 0
        : _loggingConfig_incomingRequests.ignore;
...
function logRequests(
  ...
  if (!ignoreLoggingIncomingRequests(request, loggingConfig)) {
    ...
    const serverActionLog = (0, _requestmeta.getRequestMeta)(
      request,
      "devServerActionLog",
    );
    if (serverActionLog) {
      const argsStr = (0, _serveractionlogger.formatArgs)(serverActionLog.args);
      process.stdout.write(
        `  └─ ƒ ${serverActionLog.functionName}(${argsStr}) in ${serverActionLog.duration}ms ${(0, _picocolors.dim)(serverActionLog.location)}\n`,
      );
      (0, _requestmeta.removeRequestMeta)(request, "devServerActionLog");
    }
  }
...
function logIncomingRequests(
  ...
  const url = isRSC
    ? (0, _url.stripNextRscUnionQuery)(request.url)
    : request.url;
  ...
  if (devRequestTimingInternalsEnd) {
    ...
    /* Middleware runs during the internals so we have to subtract it from the framework time */ if (
      middlewareTime
    ) {
  ...
  return writeLine(
    `${request.method} ${url} ${coloredStatus} in ${(0, _durationtostring.hrtimeBigIntDurationToString)(totalRequestTime)}${times.length > 0 ? (0, _picocolors.dim)(` (${times.map(([label, time]) => `${label}: ${(0, _durationtostring.hrtimeBigIntDurationToString)(time)}`).join(", ")})`) : ""}`,
  );
}
function logFetchMetric(fetchMetric, loggingConfig) {
  ...
  if (
    ...
    !(loggingConfig == null
      ? void 0
      : (_loggingConfig_fetches = loggingConfig.fetches) == null
        ? void 0
        : _loggingConfig_fetches.hmrRefreshes)
  ) {
  ...
  if (loggingConfig == null ? void 0 : loggingConfig.fetches) {
    ...
    if (cacheStatus === "skip" || cacheStatus === "miss") {
      writeLine(
        (0, _picocolors.gray)(
          `Cache ${cacheStatus === "skip" ? "skipped" : "missed"} reason: (${(0, _picocolors.white)(cacheReason)})`,
        ),
        2,
      );
    }
  ...
  if (cacheWarning) {
    writeLine(
      `${(0, _picocolors.yellow)((0, _picocolors.bold)("⚠"))} ${(0, _picocolors.white)(cacheWarning)}`,
      2,
    );
  }
}
