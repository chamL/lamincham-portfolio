...
function shouldIgnoreSource(sourceURL) {
  return (
    sourceURL.includes("node_modules") || // Only relevant for when Next.js is symlinked e.g. in the Next.js monorepo
...
function getModuleById(id, compilation) {
  ...
  return [...modules].find((module1) => chunkGraph.getModuleId(module1) === id);
}
function findModuleNotFoundFromError(errorMessage) {
  var _errorMessage_match;
  return errorMessage == null
    ? void 0
    : (_errorMessage_match = errorMessage.match(/'([^']+)' module/)) == null
      ? void 0
      : _errorMessage_match[1];
}
...
function getIgnoredSources(sourceMap) {
  ...
  const moduleFilenames =
    (sourceMap == null ? void 0 : sourceMap.sources) ?? [];
  for (let index = 0; index < moduleFilenames.length; index++) {
    ...
    const formattedFilePath = (0, _webpackmodulepath.formatStackFrameFile)(
      webpackSourceURL,
    );
  ...
  const ignoredSources = sourceMap.sources.map((source, index) => {
    ...
    return {
      ...
      content:
        ((_sourceMap_sourcesContent = sourceMap.sourcesContent) == null
          ? void 0
          : _sourceMap_sourcesContent[index]) ?? null,
    };
  });
...
function findOriginalSourcePositionAndContentFromCompilation(
  ...
  return (
    (module1 == null
      ? void 0
      : (_module_buildInfo = module1.buildInfo) == null
        ? void 0
        : (_module_buildInfo_importLocByPath =
              _module_buildInfo.importLocByPath) == null
          ? void 0
          : _module_buildInfo_importLocByPath.get(importedModule)) ?? null
  );
}
async function createOriginalStackFrame({
  ...
  const ignored =
    ignoredByDefault ||
    isIgnoredSource(source, sourcePosition) || // If the source file is externals, should be excluded even it's not ignored source.
  ...
  const traced = {
    ...
    methodName:
      (_frame_methodName = frame.methodName) == null
        ? void 0
        : (_frame_methodName_replace = _frame_methodName.replace(
              "__WEBPACK_DEFAULT_EXPORT__",
              "default",
            )) == null
          ? void 0
          : _frame_methodName_replace.replace("__webpack_exports__.", ""),
    arguments: [],
    ignored,
  };
  ...
  return {
    ...
    get originalCodeFrame() {
      if (originalCodeFrame === undefined) {
        originalCodeFrame = (0, _shared.getOriginalCodeFrame)(
          traced,
          sourceContent,
        );
      }
  ...
  try {
    ...
    const source =
      codeGenerationResult == null
        ? void 0
        : codeGenerationResult.sources.get("javascript");
...
async function getSource(frame, options) {
  ...
  if (nativeSourceMap !== undefined) {
    ...
    return {
      type: "file",
      sourceMap: (0, _sourcemaps.findApplicableSourceMapPayload)(
        (frame.line1 ?? 1) - 1,
        (frame.column1 ?? 1) - 1,
        sourceMapPayload,
      ),
  ...
  if (sourceURL.startsWith("file:")) {
    const sourceMap = await (0, _getsourcemapfromfile.getSourceMapFromFile)(
      sourceURL,
    );
    return sourceMap
      ? {
...
async function getOriginalStackFrame({
  ...
  const source = await getSource(frame, {
    getCompilations: () => {
      ...
      if ((!isEdgeServer && !isServer) || isAppDirectory) {
        var _clientStats;
        const compilation =
          (_clientStats = clientStats()) == null
            ? void 0
            : _clientStats.compilation;
        if (compilation) {
      ...
      if (isServer || isAppDirectory) {
        var _serverStats;
        const compilation =
          (_serverStats = serverStats()) == null
            ? void 0
            : _serverStats.compilation;
        if (compilation) {
          compilations.push(compilation);
        }
      }
      ...
      if (isEdgeServer || isAppDirectory) {
        ...
        const compilation =
          (_edgeServerStats = edgeServerStats()) == null
            ? void 0
            : _edgeServerStats.compilation;
        if (compilation) {
          compilations.push(compilation);
        }
      }
  ...
  if (
    ...
    defaultNormalizedStackFrameLocation = _path.default.relative(
      rootDirectory,
      (0, _url.fileURLToPath)(defaultNormalizedStackFrameLocation),
    );
  }
  ...
  defaultStackFrame.ignored ||= (0, _sourcemaps.sourceMapIgnoreListsEverything)(
    source.sourceMap,
  );
  ...
  return {
    originalStackFrame,
    originalCodeFrame:
      ((originalStackFrame == null ? void 0 : originalStackFrame.ignored) ??
      true)
        ? null
  ...
  return async function (req, res, next) {
    ...
    if (pathname === "/__nextjs_original-stack-frames") {
      ...
      if (isAppRelativePath) {
        ...
        const appPath = _path.default.join(
          "app",
          isSrcDir ? "src" : "",
          relativeFilePath,
        );
        openEditorResult = await (0, _launcheditor.openFileInEditor)(
          appPath,
          1,
          1,
          rootDirectory,
      ...
      if (openEditorResult.error) {
        ...
        return _middlewareresponse.middlewareResponse.internalServerError(
          res,
