 ...
 */ function shouldEmitOnceWarning(issue) {
  ...
  if (
    severity === "warning" &&
    stage === "config" &&
    (0, _utils.renderStyledStringToErrorAnsi)(issue.title).includes(
      "can't be external",
    )
  ) {
    if (onceErrorSet.has(issue)) {
  ...
  switch (route.type) {
    case "page": {
      ...
      try {
        ...
        if (entrypoints.global.document) {
          const key = (0, _entrykey.getEntryKey)(
            "pages",
            "server",
            "_document",
          );
        ...
        if (dev) {
          ...
          if (entrypoints.global.document) {
            hooks == null
              ? void 0
              : hooks.subscribeToChanges(
                  (0, _entrykey.getEntryKey)("pages", "server", "_document"),
                  false,
    ...
    default: {
      throw Object.defineProperty(
        new Error(`unknown route type ${route.type} for ${page}`),
        "__NEXT_ERROR_CODE",
        {
          value: "E316",
          enumerable: false,
...
async function handleEntrypoints({
  entrypoints,
  currentEntrypoints,
  currentEntryIssues,
  manifestLoader,
  ...
  if (middleware) {
    ...
    const triggerName = middleware.isProxy
      ? _constants.PROXY_FILENAME
      : _constants.MIDDLEWARE_FILENAME;
    async function processMiddleware() {
      ...
      const middlewareConfig =
        (_manifestLoader_getMiddlewareManifest =
          manifestLoader.getMiddlewareManifest(key)) == null
          ? void 0
          : _manifestLoader_getMiddlewareManifest.middleware["/"];
    ...
    manifestLoader.deleteMiddlewareManifest(
      (0, _entrykey.getEntryKey)("root", "server", "middleware"),
    );
  ...
  for (const client of clients) {
    ...
    for (const id of state.subscriptions.keys()) {
      if (
        !hasEntrypointForKey(
          currentEntrypoints,
          (0, _entrykey.getEntryKey)("assets", "client", id),
          assetMapper,
        )
      ) {
