...
const _inspector = /*#__PURE__*/ _interop_require_wildcard(
  require("inspector"),
);
...
const _webpackconfig = /*#__PURE__*/ _interop_require_wildcard(
...
const _getroutefromentrypoint = /*#__PURE__*/ _interop_require_default(
  require("../get-route-from-entrypoint"),
);
const _utils = require("../../build/utils");
const _utils1 = require("../../shared/lib/utils");
const _trace = require("../../trace");
const _iserror = require("../../lib/is-error");
const _ws = /*#__PURE__*/ _interop_require_default(
  require("next/dist/compiled/ws"),
);
...
const _getwebpackbundler = /*#__PURE__*/ _interop_require_default(
  require("../../shared/lib/get-webpack-bundler"),
);
...
class HotReloaderWebpack {
  ...
  async run(req, res, parsedUrl) {
    ...
    const handlePageBundleRequest = async (
      ...
      if (!pathname) return {};
      const params = (0, _hotreloadersharedutils.matchNextPageBundleRequest)(
        pathname,
      );
      if (!params) return {};
      let decodedPagePath;
      ...
      const page = (0, _denormalizepagepath.denormalizePagePath)(
        decodedPagePath,
      );
      if (
        page === "/_error" ||
        _constants1.BLOCKED_PAGES.indexOf(page) === -1
      ) {
        try {
          ...
          return await renderScriptError(
            pageBundleRes,
            (0, _iserror.getProperError)(error),
          );
        }
  ...
  onHMR(req, _socket, head, callback) {
    wsServer.handleUpgrade(req, req.socket, head, (client) => {
      ...
      if (!this.webpackHotMiddleware) {
        throw Object.defineProperty(
          new _invarianterror.InvariantError(
            "Did not start HotReloaderWebpack.",
          ),
          "__NEXT_ERROR_CODE",
          {
            value: "E804",
      ...
      (_this_onDemandEntries = this.onDemandEntries) == null
        ? void 0
        : _this_onDemandEntries.onHMR(client, () => this.hmrServerError);
          ...
          switch (payload.event) {
            ...
            case "client-hmr-latency": {
              traceChild = {
                ...
                attrs: {
                  updatedModules: payload.updatedModules.map((m) =>
                    m
                      .replace(
            ...
            case "client-full-reload": {
              ...
              if (stackTrace) {
                var _exec;
                const file =
                  (_exec = /Aborted because (.+) is not accepted/.exec(
                    stackTrace,
                  )) == null
                    ? void 0
                    : _exec[1];
                if (file) {
                  ...
                  if (
                    ...
                    const cleanedFilePath = file.slice(
                      `(${_constants.WEBPACK_LAYERS.pagesDirBrowser})/`.length,
                    );
      ...
      if (htmlRequestId) {
        (0, _debugchannel.connectReactDebugChannelForHtmlRequest)(
          htmlRequestId,
          this.sendToClient.bind(this, client),
        );
        (0, _serializederrors.sendSerializedErrorsToClientForHtmlRequest)(
          htmlRequestId,
          this.sendToClient.bind(this, client),
        );
      ...
      client.on("close", () => {
        ...
        (_this_webpackHotMiddleware = this.webpackHotMiddleware) == null
          ? void 0
          : _this_webpackHotMiddleware.deleteClient(client, htmlRequestId);
        if (htmlRequestId) {
          (0, _debugchannel.deleteReactDebugChannelForHtmlRequest)(
            htmlRequestId,
          );
        }
      });
  ...
  async clean(span) {
    return span
      .traceChild("clean")
      .traceAsyncFn(() =>
        (0, _recursivedelete.recursiveDeleteSyncWithAsyncRetries)(
          (0, _path.join)(this.dir, this.config.distDir),
          /^(cache|lock)/,
        ),
      );
  }
  async getWebpackConfig(span) {
    ...
    return webpackConfigSpan.traceAsyncFn(async () => {
      ...
      this.pagesMapping = await webpackConfigSpan
        ...
        .traceAsyncFn(() =>
          (0, _routediscovery.createPagesMapping)({
            isDev: true,
  ...
  async buildFallbackError() {
    ...
    const fallbackConfig = await (0, _webpackconfig.default)(this.dir, {
      ...
      isDevFallback: true,
      entrypoints: (
        await (0, _entries.createEntrypoints)({
          appDir: this.appDir,
          buildId: this.buildId,
          config: this.config,
          envFiles: [],
  ...
  async tracedGetVersionInfo(span) {
    ...
    return versionInfoSpan.traceAsyncFn(async () =>
      (0, _hotreloadersharedutils.getVersionInfo)(),
    );
  }
  async start() {
    ...
    const initialDevToolsConfig = await (0,
    _devtoolsconfigmiddleware.getDevToolsConfig)(this.distDir);
    ...
    for (const config of this.activeWebpackConfigs) {
      ...
      config.entry = async (...args) => {
        ...
        const outputPath =
          ((_this_multiCompiler = this.multiCompiler) == null
            ? void 0
            : _this_multiCompiler.outputPath) || "";
        const entries = (0, _ondemandentryhandler.getEntries)(outputPath);
        // @ts-ignore entry is always a function
        const entrypoints = await defaultEntry(...args);
        const isClientCompilation =
          config.name === _constants1.COMPILER_NAMES.client;
        const isNodeServerCompilation =
          config.name === _constants1.COMPILER_NAMES.server;
        const isEdgeServerCompilation =
          config.name === _constants1.COMPILER_NAMES.edgeServer;
        await Promise.all(
          Object.keys(entries).map(async (entryKey) => {
            ...
            if (
              key === _constants1.COMPILER_NAMES.client &&
              !isClientCompilation
            )
              return;
            if (
              key === _constants1.COMPILER_NAMES.server &&
              !isNodeServerCompilation
            )
              return;
            if (
              key === _constants1.COMPILER_NAMES.edgeServer &&
              !isEdgeServerCompilation
            )
              return;
            const isEntry =
              entryData.type === _ondemandentryhandler.EntryTypes.ENTRY;
            const isChildEntry =
              entryData.type === _ondemandentryhandler.EntryTypes.CHILD_ENTRY;
            // Check if the page was removed or disposed and remove it
            if (isEntry) {
              const pageExists =
                !dispose && (0, _fs.existsSync)(entryData.absolutePagePath);
              if (!pageExists) {
            ...
            if (isChildEntry) {
              if (entryData.absoluteEntryFilePath) {
                const pageExists =
                  !dispose &&
                  (0, _fs.existsSync)(entryData.absoluteEntryFilePath);
                if (!pageExists) {
                  delete entries[entryKey];
                  return;
                }
            ...
            const staticInfo = isEntry
              ? await (0,
                _getstaticinfoincludinglayouts.getStaticInfoIncludingLayouts)({
            ...
            const isServerComponent =
              isAppPath &&
              (staticInfo == null ? void 0 : staticInfo.rsc) !==
                _constants1.RSC_MODULE_TYPES.client;
            const pageType = entryData.bundlePath.startsWith("pages/")
              ? _pagetypes.PAGE_TYPES.PAGES
              : entryData.bundlePath.startsWith("app/")
                ? _pagetypes.PAGE_TYPES.APP
                : _pagetypes.PAGE_TYPES.ROOT;
            if (pageType === "pages") {
              this.hasPagesRouterEntrypoints = true;
            }
            if (pageType === "app") {
            ...
            const isInstrumentation =
              (0, _utils.isInstrumentationHookFile)(page) &&
              pageType === _pagetypes.PAGE_TYPES.ROOT;
            ...
            (0, _entries.runDependingOnPageType)({
              ...
              onEdgeServer: () => {
                ...
                if (isInstrumentation) {
                  ...
                  entrypoints[normalizedBundlePath] = (0,
                  _entries.finalizeEntrypoint)({
                    compilerType: _constants1.COMPILER_NAMES.edgeServer,
                ...
                const appDirLoader = isAppPath
                  ? (0, _entries.getAppEntry)({
                      name: bundlePath,
                      page,
                      appPaths: entryData.appPaths,
                      allNormalizedAppPaths: null,
                      pagePath: _path.posix.join(
                        _constants.APP_DIR_ALIAS,
                        (0, _path.relative)(
                          this.appDir,
                ...
                entrypoints[bundlePath] = (0, _entries.finalizeEntrypoint)({
                  ...
                  value: (0, _entries.getEdgeServerEntry)({
                    ...
                    pagesType: isAppPath
                      ...
                      : _pagetypes.PAGE_TYPES.PAGES,
                    preferredRegion:
                      staticInfo == null ? void 0 : staticInfo.preferredRegion,
                  }),
                  hasAppDir,
                });
              },
              ...
              onServer: () => {
                ...
                entries[entryKey].status = _ondemandentryhandler.BUILDING;
                let relativeRequest = (0, _path.relative)(
                  config.context,
                  entryData.absolutePagePath,
                );
                if (
                  !(0, _path.isAbsolute)(relativeRequest) &&
                ...
                if (isInstrumentation) {
                  ...
                  const pagePath = entryData.absolutePagePath.startsWith(
                    (0, _path.dirname)(require.resolve("next/package.json")),
                  )
                    ? entryData.absolutePagePath
                    : _path.posix.join(
                        _constants.APP_DIR_ALIAS,
                  ...
                  value = (0, _entries.getAppEntry)({
                    ...
                    middlewareConfig: Buffer.from(
                      JSON.stringify(
                        (staticInfo == null ? void 0 : staticInfo.middleware) ||
                          {},
                      ),
                    ).toString("base64"),
                    isGlobalNotFoundEnabled: this.config.experimental
                  ...
                  !(0, _utils.isMiddlewareFile)(page) &&
                  !(0, _isinternalcomponent.isInternalComponent)(
                    relativeRequest,
                  ) &&
                  !(0, _isinternalcomponent.isNonRoutePagesPage)(page) &&
                  !isInstrumentation
                ) {
                  value = (0, _nextrouteloader.getRouteLoaderEntry)({
        ...
        if (!this.hasPagesRouterEntrypoints) {
          delete entrypoints[
            _constants1.CLIENT_STATIC_FILES_RUNTIME_REACT_REFRESH
          ];
        }
    ...
    await Promise.all(
      Array.from(
        (0, _webpackconfig.getCacheDirectories)(this.activeWebpackConfigs),
      ).map(_cacheinvalidation.checkFileSystemCacheInvalidationAndCleanup),
    );
    this.multiCompiler = (0, _getwebpackbundler.default)()(
    ...
    this.multiCompiler.hooks.done.tap("NextjsHotReloader", () => {
      ...
      inputFileSystem == null
        ? void 0
        : (_inputFileSystem_purge = inputFileSystem.purge) == null
          ? void 0
          : _inputFileSystem_purge.call(inputFileSystem);
    });
    (0, _output.watchCompilers)(
    ...
    const trackPageChanges =
      (pageHashMap, changedItems, serverComponentChangedItems) => (stats) => {
        try {
          stats.entrypoints.forEach((entry, key) => {
            if (
              key.startsWith("pages/") ||
              key.startsWith("app/") ||
              (0, _utils.isMiddlewareFilename)(key)
            ) {
              ...
              entry.chunks.forEach((chunk) => {
                if (chunk.id === key) {
                  ...
                  modsIterable.forEach((mod) => {
                    if (
                      mod.resource &&
                      mod.resource.replace(/\\/g, "/").includes(key) && // Shouldn't match CSS modules, etc.
                      ...
                      if (
                        ...
                        (mod == null
                          ? void 0
                          : (_mod_buildInfo = mod.buildInfo) == null
                            ? void 0
                            : (_mod_buildInfo_rsc = _mod_buildInfo.rsc) == null
                              ? void 0
                              : _mod_buildInfo_rsc.type) !== "client"
                      ) {
                      ...
                      if (
                        ...
                        (mod == null
                          ? void 0
                          : (_mod_buildInfo1 = mod.buildInfo) == null
                            ? void 0
                            : (_mod_buildInfo_rsc1 = _mod_buildInfo1.rsc) ==
                                null
                              ? void 0
                              : _mod_buildInfo_rsc1.type) !== "client"
                      ) {
                        chunksHashServerLayer.add(hash);
                      }
                      chunksHash.add(hash);
    ...
    this.multiCompiler.compilers[1].hooks.done.tap(
      ...
      (stats) => {
        ...
        if (this.appDir) {
          ...
          const diffChunkNames = (0, _utils.difference)(
    ...
    this.multiCompiler.hooks.done.tap("NextjsHotReloaderForServer", (stats) => {
      ...
      this.reloadAfterInvalidation = false;
      const serverOnlyChanges = (0, _utils.difference)(
        changedServerPages,
        changedClientPages,
      );
      const edgeServerOnlyChanges = (0, _utils.difference)(
        changedEdgeServerPages,
        changedClientPages,
      );
      ...
      (stats) => {
        ...
        const chunkNames = new Set(
          [...compilation.namedChunks.keys()].filter(
            (name) => !!(0, _getroutefromentrypoint.default)(name),
          ),
        );
    ...
    this.webpackHotMiddleware = new _hotmiddleware.WebpackHotMiddleware(
    ...
    this.middlewares.push(
      ...
      (0, _getnexterrorfeedbackmiddleware.getNextErrorFeedbackMiddleware)(
        this.telemetry,
      ),
      ...
      (0, _devtoolsconfigmiddleware.devToolsConfigMiddleware)({
        ...
        sendUpdateSignal: (data) => {
          ...
          (_this_webpackHotMiddleware = this.webpackHotMiddleware) == null
            ? void 0
            : _this_webpackHotMiddleware.updateDevToolsConfig(data);
          this.send({
        ...
        ? [
            (0, _getmcpmiddleware.getMcpMiddleware)({
              ...
              getActiveConnectionCount: () => {
                var _this_webpackHotMiddleware;
                return (
                  ((_this_webpackHotMiddleware = this.webpackHotMiddleware) ==
                  null
                    ? void 0
                    : _this_webpackHotMiddleware.getClientCount()) ?? 0
                );
              },
  ...
  invalidate(
    ...
    if (outputPath) {
      var _getInvalidator;
      (_getInvalidator = (0, _ondemandentryhandler.getInvalidator)(
        outputPath,
      )) == null
        ? void 0
        : _getInvalidator.invalidate();
    }
  }
  async getCompilationErrors(page) {
    ...
    const getErrors = ({ compilation }) => {
      ...
      return ((_failedPages_normalizedPage = failedPages[normalizedPage]) ==
      null
        ? void 0
        : _failedPages_normalizedPage.length) > 0
    ...
    if (this.clientError) {
      ...
      (_this_clientStats = this.clientStats) == null
        ? void 0
        : _this_clientStats.hasErrors()
    ) {
      return getErrors(this.clientStats);
    } else if (
      (_this_serverStats = this.serverStats) == null
        ? void 0
        : _this_serverStats.hasErrors()
    ) {
      return getErrors(this.serverStats);
    } else if (
      (_this_edgeServerStats = this.edgeServerStats) == null
        ? void 0
        : _this_edgeServerStats.hasErrors()
    ) {
  ...
  setCacheStatus(status, htmlRequestId) {
    ...
    const client =
      (_this_webpackHotMiddleware = this.webpackHotMiddleware) == null
        ? void 0
        : _this_webpackHotMiddleware.getClient(htmlRequestId);
    if (client !== undefined) {
      this.sendToClient(client, {
  ...
  setReactDebugChannel(debugChannel, htmlRequestId, requestId) {
    ...
    if (htmlRequestId === requestId) {
      ...
      if (client) {
        ...
        (0, _debugchannel.connectReactDebugChannel)(
          htmlRequestId,
          debugChannel,
          this.sendToClient.bind(this, client),
        );
        ...
        // the debug channel.
        (0, _debugchannel.setReactDebugChannelForHtmlRequest)(
          htmlRequestId,
          debugChannel,
        );
      }
  ...
  sendErrorsToBrowser(errorsRscStream, htmlRequestId) {
    ...
    if (client) {
      ...
      (0, _serializederrors.sendSerializedErrorsToClient)(
        errorsRscStream,
        this.sendToClient.bind(this, client),
      );
    } else {
      ...
      // client connects.
      (0, _serializederrors.setErrorsRscStreamForHtmlRequest)(
        htmlRequestId,
        errorsRscStream,
      );
    }
  ...
  async ensurePage({ page, clientOnly, appPaths, definition, isApp, url }) {
    return this.hotReloaderSpan
      ...
      .traceAsyncFn(async () => {
        ...
        return (_this_onDemandEntries = this.onDemandEntries) == null
          ? void 0
          : _this_onDemandEntries.ensurePage({
              page,
              appPaths,
              definition,
              isApp,
  ...
  close() {
    ...
    (_this_webpackHotMiddleware = this.webpackHotMiddleware) == null
      ? void 0
      : _this_webpackHotMiddleware.close();
  }
}
