...
const _hotreloaderwebpack = /*#__PURE__*/ _interop_require_default(
  require("./hot-reloader-webpack"),
);
...
class HotReloaderRspack extends _hotreloaderwebpack.default {
  async afterCompile(multiCompiler) {
    ...
    await rspackStartSpan.traceAsyncFn(async () => {
      ...
      this.builtEntriesCachePath = _path.default.join(
        this.distDir,
        "cache",
        "rspack",
      ...
      const hasBuiltEntriesCache = await _promises.default
      ...
      if (hasBuiltEntriesCache) {
        try {
          ...
          await Promise.all(
            Object.keys(builtEntries).map(async (entryKey) => {
              ...
              if (isEntry) {
                const pageExists =
                  !entryData.dispose &&
                  (await _promises.default
                    .access(entryData.absolutePagePath)
                    .then(
                      () => true,
                      () => false,
                    ));
              ...
              if (isChildEntry) {
                if (entryData.absoluteEntryFilePath) {
                  const pageExists =
                    !entryData.dispose &&
                    (await _promises.default
                      .access(entryData.absoluteEntryFilePath)
                      .then(
                        () => true,
                        () => false,
                      ));
          ...
          Object.assign(
            (0, _ondemandentryhandler.getEntries)(multiCompiler.outputPath),
            builtEntries,
          );
        } catch (error) {
  ...
  async ensurePage({ page, clientOnly, appPaths, definition, isApp, url }) {
    ...
    const entries = (0, _ondemandentryhandler.getEntries)(
      this.multiCompiler.outputPath,
    );
    const builtEntries = {};
    await Promise.all(
    ...
    const hasBuitEntriesCache = await _promises.default
      .access(this.builtEntriesCachePath)
      .then(
        () => true,
        () => false,
    ...
    try {
      if (!hasBuitEntriesCache) {
        await _promises.default.mkdir(
          _path.default.dirname(this.builtEntriesCachePath),
          {
            recursive: true,
          },
        );
  ...
  constructor(...args) {
    (super(...args),
      (this.isClientCacheEnabled = false),
      (this.isServerCacheEnabled = false),
      (this.isEdgeServerCacheEnabled = false));
  }
}
async function calculateFileHash(filePath, algorithm = "sha256") {
  if (
    !(await _promises.default.access(filePath).then(
      () => true,
      () => false,
    ))
  ) {
