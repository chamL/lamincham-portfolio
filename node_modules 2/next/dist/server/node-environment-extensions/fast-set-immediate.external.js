...
const originals =
  globalThis[FAST_SET_IMMEDIATE_ORIGINALS_KEY] ??
  (globalThis[FAST_SET_IMMEDIATE_ORIGINALS_KEY] = {
...
const originalSetImmediatePromisify =
  typeof originalSetImmediate === "function"
    ? originalSetImmediate[_nodeutil.promisify.custom] // and won't ever enable the patch, so this can be a dummy value
    : undefined;
function install() {
...
/** Execute one immediate, and schedule a check for more (in case there's others in the queue) */ function performWork(
  execution,
) {
  if (execution.state === 4) {
    return;
  ...
  queueMicrotask(() => {
    if (didThrow) {
      debug == null
        ? void 0
        : debug(
            "scheduler :: rethrowing sync error from immediate in microtask",
          );
      throw thrownValue;
    }
  });
...
function patchedNextTick() {
  if (currentExecution === null) {
    return originalNextTick.apply(
      null, // @ts-expect-error: this is valid, but typescript doesn't get it
      arguments,
    );
  }
  if (arguments.length === 0 || typeof arguments[0] !== "function") {
    ...
    originalNextTick.apply(
      null, // @ts-expect-error: explicitly passing arguments that we know are invalid
      arguments,
    );
...
function patchedSetImmediate() {
  ...
  if (arguments.length === 0 || typeof arguments[0] !== "function") {
    ...
    bail(
      ...
      Object.defineProperty(
        new _invarianterror.InvariantError(
          "Expected setImmediate to reject invalid arguments",
        ),
        "__NEXT_ERROR_CODE",
        {
          value: "E965",
  ...
  const callbackWithAsyncContext = (0, _asynclocalstorage.bindSnapshot)(
    callback,
  );
