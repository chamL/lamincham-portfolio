...
const _path = /*#__PURE__*/ _interop_require_default(
  require("../../shared/lib/isomorphic/path"),
);
...
class DevRouteMatcherManager
  extends _defaultroutematchermanager.DefaultRouteMatcherManager
{
  ...
  validate(pathname, matcher, options) {
    ...
    if (
      ...
      matcher.duplicated.some(
        (duplicate) =>
          duplicate.definition.kind === _routekind.RouteKind.APP_PAGE ||
          duplicate.definition.kind === _routekind.RouteKind.APP_ROUTE,
      ) &&
      matcher.duplicated.some(
        (duplicate) =>
          duplicate.definition.kind === _routekind.RouteKind.PAGES ||
          duplicate.definition.kind === _routekind.RouteKind.PAGES_API,
      )
    ) {
      return null;
    }
  ...
  async reload() {
    ...
    for (const [pathname, matchers] of Object.entries(
      ...
      _log.warn(
        `Duplicate page detected. ${matchers.map((matcher) => (0, _picocolors.cyan)(_path.default.relative(this.dir, matcher.definition.filename))).join(" and ")} resolve to ${(0, _picocolors.cyan)(pathname)}`,
      );
    }
  }
}
