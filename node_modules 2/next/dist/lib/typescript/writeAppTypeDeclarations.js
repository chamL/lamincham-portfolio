...
async function writeAppTypeDeclarations({
  ...
  const routeTypesPath = _path.default.posix.join(
    distDir.replaceAll(_path.default.win32.sep, _path.default.posix.sep),
    "types/routes.d.ts",
  );
  // Use ESM import instead of triple-slash reference for better ESLint compatibility
  lines.push(`import "./${routeTypesPath}";`);
  if (strictRouteTypes) {
    const cacheLifePath = _path.default.posix.join(
      distDir.replaceAll(_path.default.win32.sep, _path.default.posix.sep),
      "types/cache-life.d.ts",
    );
    lines.push(`import "./${cacheLifePath}";`);
    const routeValidatorPath = _path.default.posix.join(
      distDir.replaceAll(_path.default.win32.sep, _path.default.posix.sep),
      "types/validator.ts",
    );
    lines.push(`import "./${routeValidatorPath}";`);
    if (typedRoutes === true) {
      const linkTypesPath = _path.default.posix.join(
        distDir.replaceAll(_path.default.win32.sep, _path.default.posix.sep),
        "types/link.d.ts",
      );
