...
function Form({
  ...
  const isPrefetchEnabled = !!router && isNavigatingForm && prefetch === null; // if we don't have an action path, we can't prefetch anything.
  const observeFormVisibilityOnMount = (0, _react.useCallback)(
    (element) => {
      if (isPrefetchEnabled && router !== null) {
        (0, _links.mountFormInstance)(
          element,
          actionProp,
          router, // We default to PPR. We'll discover whether or not the route supports it with the initial prefetch.
...
function onFormSubmit(
  event,
  { actionHref, onSubmit, replace, scroll, router },
) {
  ...
  const targetUrl = (0, _formshared.createFormSubmitDestinationUrl)(
    action,
    formElement,
  );
