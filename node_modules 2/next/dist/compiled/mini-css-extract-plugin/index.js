(() => {
  ...
  (() => {
    ...
    var s = _interopRequireDefault(__nccwpck_require__(804));
    var i = __nccwpck_require__(362);
    function _interopRequireDefault(e) {
      return e && e.__esModule ? e : { default: e };
    }
    ...
    e.pluginSymbol = o;
    const a = "[name].css";
    const u = new Set([i.MODULE_TYPE]);
    const l = { sources: new Map(), runtimeRequirements: new Set() };
    const d = new WeakMap();
    const c = new WeakMap();
    const p = new WeakSet();
    class MiniCssExtractPlugin {
      ...
      constructor(e = {}) {
        (0, t.validate)(s.default, e, { baseDataPath: "options" });
      ...
      apply(e) {
        ...
        e.hooks.thisCompilation.tap(r, (n) => {
          ...
          n.hooks.renderManifest.tap(r, (s, { chunk: o }) => {
            ...
            const l = Array.from(this.getChunkModules(o, a)).filter(
              (e) => e.type === i.MODULE_TYPE,
            );
            const d = o.canBeInitial()
              ...
              s.push({
                render: () =>
                  this.renderContentAsset(
                    ...
                    d,
                    { contentHashType: i.MODULE_TYPE, chunk: o },
                  ),
                filenameTemplate: d,
                pathOptions: { chunk: o, contentHashType: i.MODULE_TYPE },
                identifier: `${r}.${o.id}`,
          ...
          n.hooks.contentHash.tap(r, (t) => {
            ...
            const o = this.sortModules(
              n,
              t,
              r.getChunkModulesIterableBySourceType(t, i.MODULE_TYPE),
              n.runtimeTemplate.requestShortener,
            );
            if (o) {
          ...
          const getCssChunkObject = (e, t) => {
            ...
            for (const t of e.getAllAsyncChunks()) {
              const e = s.getOrderedChunkModulesIterable(
                t,
          ...
          class CssLoadingRuntimeModule extends l {
            ...
            generate() {
              ...
              const {
                runtimeTemplate: n,
                outputOptions: { crossOriginLoading: s },
              } = this.compilation;
              const i = getCssChunkObject(e, this.compilation);
              const r =
                t.has(u.ensureChunkHandlers) && Object.keys(i).length > 0;
              const a = t.has(u.hmrDownloadUpdateHandlers);
              if (!r && !a) {
                return null;
              }
              return o.asString([
                `var createStylesheet = ${n.basicFunction(
                  ...
                  [
                    ...
                    this.runtimeOptions.linkType
                      ...
                      : "",
                    `var onLinkComplete = ${n.basicFunction("event", ["// avoid mem leaks.", "linkTag.onerror = linkTag.onload = null;", "if (event.type === 'load') {", o.indent(["resolve();"]), "} else {", o.indent(["var errorType = event && (event.type === 'load' ? 'missing' : event.type);", "var realHref = event && event.target && event.target.href || fullhref;", 'var err = new Error("Loading CSS chunk " + chunkId + " failed.\\n(" + realHref + ")");', 'err.code = "CSS_CHUNK_LOAD_FAILED";', "err.type = errorType;", "err.request = realHref;", "linkTag.parentNode.removeChild(linkTag)", "reject(err);"]), "}"])}`,
                    ...
                    s
                      ? o.asString([
                          ...
                          o.indent(
                            `linkTag.crossOrigin = ${JSON.stringify(s)};`,
                          ),
                          "}",
                        ])
                      : "",
                    typeof this.runtimeOptions.insert !== "undefined"
                      ...
                      : o.asString(["document.head.appendChild(linkTag);"]),
                    "return linkTag;",
                  ],
                )};`,
                `var findStylesheet = ${n.basicFunction("href, fullhref", ['var existingLinkTags = document.getElementsByTagName("link");', "for(var i = 0; i < existingLinkTags.length; i++) {", o.indent(["var tag = existingLinkTags[i];", 'var dataHref = tag.getAttribute("data-href") || tag.getAttribute("href");', 'if(tag.rel === "stylesheet" && (dataHref === href || dataHref === fullhref)) return tag;']), "}", 'var existingStyleTags = document.getElementsByTagName("style");', "for(var i = 0; i < existingStyleTags.length; i++) {", o.indent(["var tag = existingStyleTags[i];", 'var dataHref = tag.getAttribute("data-href");', "if(dataHref === href || dataHref === fullhref) return tag;"]), "}"])};`,
                `var loadStylesheet = ${n.basicFunction("chunkId", `return new Promise(${n.basicFunction("resolve, reject", [`var href = ${u.require}.miniCssF(chunkId);`, `var fullhref = ${u.publicPath} + href;`, "if(findStylesheet(href, fullhref)) return resolve();", "createStylesheet(chunkId, fullhref, resolve, reject);"])});`)}`,
                r
                  ? o.asString([
                ...
                a
                  ? o.asString([
                      "var oldTags = [];",
                      "var newTags = [];",
                      `var applyHandler = ${n.basicFunction("options", [`return { dispose: ${n.basicFunction("", ["for(var i = 0; i < oldTags.length; i++) {", o.indent(["var oldTag = oldTags[i];", "if(oldTag.parentNode) oldTag.parentNode.removeChild(oldTag);"]), "}", "oldTags.length = 0;"])}, apply: ${n.basicFunction("", ['for(var i = 0; i < newTags.length; i++) newTags[i].rel = "stylesheet";', "newTags.length = 0;"])} };`])}`,
                      `${u.hmrDownloadUpdateHandlers}.miniCss = ${n.basicFunction("chunkIds, removedChunks, removedModules, promises, applyHandlers, updatedModulesList", ["applyHandlers.push(applyHandler);", `chunkIds.forEach(${n.basicFunction("chunkId", [`var href = ${u.require}.miniCssF(chunkId);`, `var fullhref = ${u.publicPath} + href;`, "var oldTag = findStylesheet(href, fullhref);", "if(!oldTag) return;", `promises.push(new Promise(${n.basicFunction("resolve, reject", [`var tag = createStylesheet(chunkId, fullhref, ${n.basicFunction("", ['tag.as = "style";', 'tag.rel = "preload";', "resolve();"])}, reject);`, "oldTags.push(oldTag);", "newTags.push(tag);"])}));`])});`])}`,
                    ])
                  : "// no hmr",
              ]);
            }
          ...
          const handler = (e, t) => {
            ...
            n.addRuntimeModule(
              e,
              new d.GetChunkFilenameRuntimeModule(
                i.MODULE_TYPE,
                "mini-css",
                `${u.require}.miniCssF`,
                (e) => {
            ...
            n.addRuntimeModule(
              e,
              new CssLoadingRuntimeModule(t, this.runtimeOptions),
            );
          };
          n.hooks.runtimeRequirementInTree
            .for(u.ensureChunkHandlers)
            .tap(r, handler);
          n.hooks.runtimeRequirementInTree
            .for(u.hmrDownloadUpdateHandlers)
            .tap(r, handler);
        });
      }
      getChunkModules(e, t) {
      ...
      sortModules(e, t, n, s) {
        ...
        const o = [...n];
        const a = new Map(o.map((e) => [e, new Set()]));
        const u = new Map(o.map((e) => [e, new Map()]));
        const l = Array.from(t.groupsIterable, (e) => {
          const t = o
            .map((t) => ({ module: t, index: e.getModulePostOrderIndex(t) }))
