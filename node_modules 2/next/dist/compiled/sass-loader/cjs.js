(function () {
  ...
  var __webpack_modules__ = {
    ...
    371: function (e, t, s) {
      ...
      var r = _interopRequireDefault(s(33));
      var a = s(159);
      ...
      async function loader(e) {
        ...
        const l =
          typeof i.compileStringAsync === "undefined"
            ? "legacy"
            : typeof t.api === "undefined"
              ? "modern"
              : t.api;
        const p = await (0, a.getSassOptions)(this, t, e, i, c, l);
        const u =
          typeof t.webpackImporter === "boolean" ? t.webpackImporter : true;
        if (u) {
          const e = l === "modern" || l === "modern-compiler";
          if (!e) {
            const { includePaths: e } = p;
            p.importer.push((0, a.getWebpackImporter)(this, i, e));
          } else {
            p.importers.push((0, a.getModernWebpackImporter)(this, i, []));
          }
        }
        let d;
        try {
          d = (0, a.getCompileFn)(this, i, l);
        } catch (e) {
          s(e);
          return;
        }
        ...
        let m = f.sourceMap ? f.sourceMap : f.map ? JSON.parse(f.map) : null;
        if (m && c) {
          m = (0, a.normalizeSourceMap)(m, this.rootContext);
        }
        if (typeof f.loadedUrls !== "undefined") {
          f.loadedUrls
          ...
          typeof f.stats !== "undefined" &&
          typeof f.stats.includedFiles !== "undefined"
        ) {
          f.stats.includedFiles.forEach((e) => {
    ...
    159: function (__unused_webpack_module, exports, __nccwpck_require__) {
      ...
      function getDefaultSassImplementation() {
        ...
        try {
          eval("require").resolve("sass-embedded");
      ...
      function proxyCustomImporters(e, t) {
        return [].concat(e).map(
          (e) =>
            function proxyImporter(...s) {
              const n = { ...this, webpackLoaderContext: t };
              return e.apply(n, s);
            },
        );
      }
      async function getSassOptions(e, t, s, n, o, r) {
        const a = t.sassOptions
        ...
        if (!i.logger) {
          ...
          const formatSpan = (e) =>
            `Warning on line ${e.start.line}, column ${e.start.column} of ${e.url || "-"}:${e.start.line}:${e.start.column}:\n`;
          const formatDebugSpan = (e) =>
            `[debug:${e.start.line}:${e.start.column}] `;
          i.logger = {
            debug(e, t) {
              let s = "";
              if (t.span) {
        ...
        const c = r === "modern" || r === "modern-compiler";
        const { resourcePath: l } = e;
        if (c) {
          i.url = _url.default.pathToFileURL(l);
          if (!i.style && isProductionLikeMode(e)) {
            i.style = "compressed";
          }
          if (o) {
            i.sourceMap = true;
            i.sourceMapIncludeSources = true;
          }
          if (typeof i.syntax === "undefined") {
            const e = _path.default.extname(l);
          ...
          i.loadPaths = []
            .concat(
              (i.loadPaths ? i.loadPaths.slice() : []).map((e) =>
                _path.default.isAbsolute(e)
                  ? e
                  : _path.default.join(process.cwd(), e),
              ),
            )
            .concat(
          ...
          i.importers = i.importers
            ? Array.isArray(i.importers)
              ? i.importers.slice()
              : [i.importers]
            : [];
        } else {
          i.file = l;
          if (!i.outputStyle && isProductionLikeMode(e)) {
            i.outputStyle = "compressed";
          }
          if (o) {
            i.sourceMap = true;
            i.outFile = _path.default.join(e.rootContext, "style.css.map");
            i.sourceMapContents = true;
            i.omitSourceMapUrl = true;
            i.sourceMapEmbed = false;
          }
          const s = _path.default.extname(l);
          if (
            s &&
            s.toLowerCase() === ".sass" &&
            typeof i.indentedSyntax === "undefined"
          ) {
            i.indentedSyntax = true;
          } else {
            i.indentedSyntax = Boolean(i.indentedSyntax);
          }
          i.importer = i.importer
            ? proxyCustomImporters(
                Array.isArray(i.importer) ? i.importer.slice() : [i.importer],
                e,
              )
            : [];
          if (t.webpackImporter === false && i.importer.length === 0) {
            i.importer = undefined;
          }
          i.includePaths = []
            .concat(process.cwd())
            .concat(
              (i.includePaths ? i.includePaths.slice() : []).map((e) =>
                _path.default.isAbsolute(e)
                  ? e
                  : _path.default.join(process.cwd(), e),
              ),
      ...
      function getPossibleRequests(e, t = false, s = false) {
        ...
        return [
          ...new Set(
            []
              .concat(s ? [`${a}_${c}.import${o}`, `${a}${c}.import${o}`] : [])
              .concat([`${a}_${i}`, `${a}${i}`])
              .concat(t ? [e] : []),
          ),
        ];
      ...
      function getWebpackResolver(e, t, s = []) {
        const n = t && typeof t.compileStringAsync !== "undefined";
        ...
        return (e, t, c) => {
          ...
          const d =
            ...
            !IS_PKG_SCHEME.test(t) &&
            !p &&
            !l.startsWith("/") &&
            !IS_NATIVE_WIN32_PATH.test(l);
          if (s.length > 0 && d) {
      ...
      function getModernWebpackImporter(e, t, s) {
        ...
        return {
          async canonicalize(t, s) {
            ...
            const r = s.containingUrl
              ? _url.default.fileURLToPath(s.containingUrl.toString())
              : e.resourcePath;
            let a;
            try {
              a = await n(r, t, o);
          ...
          async load(t) {
            ...
            try {
              ...
              return { contents: s, syntax: n, sourceMapUrl: t };
            } catch (e) {
              return null;
            }
          },
      ...
      function getWebpackImporter(e, t, s) {
        ...
        return function importer(t, s, o) {
          ...
          n(s, t, typeof r === "undefined" ? true : r)
            .then((t) => {
              e.addDependency(_path.default.normalize(t));
              o({ file: t.replace(MATCH_CSS, "") });
            })
            .catch(() => {
              o({ file: t });
            });
      ...
      function getCompileFn(e, t, s) {
        if (typeof t.compileStringAsync !== "undefined") {
          if (s === "modern") {
            return (e) => {
              const { data: s, ...n } = e;
      ...
      function errorFactory(e) {
        ...
        if (e.formatted) {
          ...
          t = (e.message || e.toString()).replace(/^(.+)?Error: /, "");
        }
        const s = new Error(t, { cause: e });
        s.name = e.name;
        s.stack = null;
        return s;
      }
