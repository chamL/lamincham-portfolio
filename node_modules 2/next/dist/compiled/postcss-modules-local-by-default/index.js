(() => {
  var e = {
    ...
    742: (e, t, r) => {
      ...
      const o = "cssmodules-pure-no-check";
      const a = "cssmodules-pure-ignore";
      const isSpacing = (e) => e.type === "combinator" && e.value === " ";
      const isPureCheckDisabled = (e) => {
        for (const t of e.nodes) {
          if (t.type !== "comment") {
            return false;
          }
          if (t.text.trim().startsWith(o)) {
            return true;
          }
        }
        return false;
      };
      function getIgnoreComment(e) {
        if (!e.parent) {
          return;
        }
        const t = e.parent.index(e);
        for (let r = t - 1; r >= 0; r--) {
          const t = e.parent.nodes[r];
          if (t.type === "comment") {
            if (t.text.trimStart().startsWith(a)) {
              return t;
            }
          } else {
            break;
      ...
      const u = Symbol("is-pure-selector");
      function localizeNode(e, t, r) {
        const transform = (t, i) => {
          if (i.ignoreNextSpacing && !isSpacing(t)) {
            throw new Error("Missing whitespace after " + i.ignoreNextSpacing);
          }
          if (i.enforceNoSpacing && isSpacing(t)) {
            throw new Error("Missing whitespace before " + i.enforceNoSpacing);
          }
          let s;
          switch (t.type) {
            case "root": {
              let e;
              i.hasPureGlobals = false;
              s = t.nodes.map((r) => {
                const n = {
                  global: i.global,
                  lastWasSpacing: true,
                ...
                if (!n.hasLocals) {
                  i.hasPureGlobals = true;
                }
                return r;
              });
              i.global = e;
              t.nodes = normalizeNodeArray(s);
              break;
            }
            case "selector": {
              s = t.map((e) => transform(e, i));
              t = t.clone();
              t.nodes = normalizeNodeArray(s);
              break;
            }
            case "combinator": {
              if (isSpacing(t)) {
                if (i.ignoreNextSpacing) {
                  i.ignoreNextSpacing = false;
                  i.lastWasSpacing = false;
                  i.enforceNoSpacing = false;
                  return null;
                }
                i.lastWasSpacing = true;
                return t;
              }
              break;
            }
            case "pseudo": {
              let e;
              const r = !!t.length;
              const o = t.value === ":local" || t.value === ":global";
              const a = t.value === ":import" || t.value === ":export";
              if (a) {
                i.hasLocals = true;
              } else if (r) {
                if (o) {
                  if (t.nodes.length === 0) {
                    throw new Error(`${t.value}() can't be empty`);
                  }
                  if (i.inside) {
                    throw new Error(
                      `A ${t.value} is not allowed inside of a ${i.inside}(...)`,
                    );
                  }
                  e = {
                    global: t.value === ":global",
                    inside: t.value,
                    hasLocals: false,
                    explicit: true,
                  };
                  ...
                  if (s.length) {
                    const { before: e, after: r } = t.spaces;
                    const n = s[0];
                    const i = s[s.length - 1];
                    n.spaces = { before: e, after: n.spaces.after };
                  ...
                  s = t.map((t) => {
                    const r = { ...e, enforceNoSpacing: false };
                    const n = transform(t, r);
                    e.global = r.global;
                ...
                const e = !!t.spaces.before;
                i.ignoreNextSpacing = i.lastWasSpacing ? t.value : false;
                i.enforceNoSpacing = i.lastWasSpacing ? false : t.value;
                i.global = t.value === ":global";
                i.explicit = true;
                return e ? n.combinator({ value: " " }) : null;
              }
              break;
            }
            case "id":
            case "class": {
              ...
              const e = r.has(t.value);
              const s = e && i.explicit;
              if (!e || s) {
                const e = t.clone();
                e.spaces = { before: "", after: "" };
                t = n.pseudo({ value: ":local", nodes: [e], spaces: t.spaces });
                i.hasLocals = true;
              }
              break;
            }
      ...
      function localizeDeclarationValues(e, t, r) {
        ...
        n.walk((t, n, i) => {
          ...
          if (t.type === "word" && c.includes(t.value.toLowerCase())) {
            return;
          }
          const s = {
            options: r.options,
      ...
      function localizeDeclaration(e, t) {
        const r = /animation(-name)?$/i.test(e.prop);
        if (r) {
          let r = {};
          const n = i(e.value).walk((e) => {
            if (e.type === "div") {
              r = {};
              return;
            } else if (
              e.type === "function" &&
              e.value.toLowerCase() === "local" &&
              e.nodes.length === 1
            ) {
              e.type = "word";
              e.value = e.nodes[0].value;
              return localizeDeclNode(e, {
                options: t.options,
                global: t.global,
              ...
              if (e.value.toLowerCase() === "global" && e.nodes.length === 1) {
                e.type = "word";
                e.value = e.nodes[0].value;
              }
              return false;
            ...
            const n = e.type === "word" ? e.value.toLowerCase() : null;
            let i = false;
            if (n && l.test(n)) {
              if ("$" + n in f) {
                r["$" + n] = "$" + n in r ? r["$" + n] + 1 : 0;
        ...
        if (/url\(/i.test(e.value)) {
          return localizeDeclarationValues(false, e, t);
        }
      }
      const isPureSelector = (e, t) => {
        if (!t.parent || t.type === "root") {
          return !e.hasPureGlobals;
        }
        if (t.type === "rule" && t[u]) {
          return t[u] || isPureSelector(e, t.parent);
        }
        return !e.hasPureGlobals || isPureSelector(e, t.parent);
      };
      const isNodeWithoutDeclarations = (e) => {
        if (e.nodes.length > 0) {
          return !e.nodes.every(
            (e) =>
              e.type === "rule" ||
              (e.type === "atrule" && !isNodeWithoutDeclarations(e)),
          );
        }
        return true;
      };
          ...
          prepare() {
            ...
            return {
              Once(i) {
                ...
                const a = t && !isPureCheckDisabled(i);
                ...
                i.walkAtRules((i) => {
                  if (/keyframes$/i.test(i.name)) {
                    const t = /^\s*:global\s*\((.+)\)\s*$/.exec(i.params);
                    const s = /^\s*:local\s*\((.+)\)\s*$/.exec(i.params);
                    let o = r;
                    if (t) {
                      if (a) {
                        const e = getIgnoreComment(i);
                        if (!e) {
                          throw i.error(
                    ...
                    if (i.params) {
                      const r = t ? getIgnoreComment(i) : undefined;
                      if (r) {
                        r.remove();
                      }
                      i.params = i.params
                        .split("to")
                        .map((t) => {
                          const s = t.trim().slice(1, -1).trim();
                          const o = localizeNode(s, e.mode, n);
                          o.options = e;
                          o.localAliasMap = n;
                          if (a && o.hasPureGlobals && !r) {
                            throw i.error(
                              'Selector in at-rule"' +
                                s +
                                '" is not pure ' +
                ...
                i.walkRules((r) => {
                  ...
                  i.options = e;
                  i.localAliasMap = n;
                  const s = a ? getIgnoreComment(r) : undefined;
                  const o = a && !isPureSelector(i, r);
                  if (o && isNodeWithoutDeclarations(r) && !s) {
                    throw r.error(
                      'Selector "' +
                        r.selector +
                        '" is not pure ' +
    ...
    133: (e, t, r) => {
      ...
      t.__esModule = true;
      t["default"] = void 0;
      var n = _interopRequireDefault(r(128));
      var i = _interopRequireWildcard(r(60));
    ...
    553: (e, t, r) => {
      ...
      t.__esModule = true;
      t["default"] = void 0;
      var n = _interopRequireDefault(r(478));
      var i = _interopRequireDefault(r(193));
      var s = _interopRequireDefault(r(692));
      var o = _interopRequireDefault(r(416));
      var a = _interopRequireDefault(r(114));
      var u = _interopRequireDefault(r(830));
      var c = _interopRequireDefault(r(54));
      var l = _interopRequireDefault(r(330));
      var f = _interopRequireWildcard(r(767));
      var p = _interopRequireDefault(r(627));
      var d = _interopRequireDefault(r(242));
      var h = _interopRequireDefault(r(426));
      var v = _interopRequireDefault(r(549));
      var _ = _interopRequireWildcard(r(999));
      var y = _interopRequireWildcard(r(530));
      var g = _interopRequireWildcard(r(763));
      var b = r(362);
      var S, m;
      function _getRequireWildcardCache(e) {
        if (typeof WeakMap !== "function") return null;
      ...
      var O =
        ...
        (S[y.tab] = true),
        S);
      var w = Object.assign({}, O, ((m = {}), (m[y.comment] = true), m));
      ...
      var P = (function () {
        ...
        e.attribute = function attribute() {
          ...
          while (i < r) {
            ...
            switch (c[_.FIELDS.TYPE]) {
              ...
              case y.dollar:
                if (a === "value") {
                  var g = (0, b.getProp)(n, "raws", "value");
                  n.value += "$";
                  if (g) {
                    n.raws.value = g + "$";
                  }
              ...
              case y.word:
                if (
                  ...
                  var m = (0, b.unesc)(l);
                  var O = (0, b.getProp)(n, "raws", "value") || "";
                  var w = n.value || "";
                  n.value = w + m;
                  n.quoteMark = null;
                  if (m !== l || O) {
              ...
              case y.comment:
                if (a) {
                  if (
                    ...
                    var I =
                      (0, b.getProp)(n, "raws", "spaces", a, "after") || D;
                    (0, b.ensureObject)(n, "raws", "spaces", a);
                    n.raws.spaces[a].after = I + l;
                  } else {
                    var L = n[a] || "";
                    var q = (0, b.getProp)(n, "raws", a) || L;
                    (0, b.ensureObject)(n, "raws");
                    n.raws[a] = q + l;
                  }
                } else {
        ...
        e.parseWhitespaceEquivalentTokens =
          function parseWhitespaceEquivalentTokens(e) {
            ...
            var i = undefined;
            do {
              if (O[this.currToken[_.FIELDS.TYPE]]) {
                if (!this.options.lossy) {
                  n += this.content();
                }
          ...
          function convertWhitespaceNodesToSpace(e, t) {
            ...
            e.forEach(function (e) {
              ...
              i +=
                s +
                e.value +
                r.lossySpace(e.rawSpaceAfter, t && o.length === 0);
            });
            if (i === n) {
              i = undefined;
            }
        ...
        e.combinator = function combinator() {
          ...
          if (
            t < 0 ||
            this.tokens[t][_.FIELDS.TYPE] === y.comma ||
            this.tokens[t][_.FIELDS.TYPE] === y.closeParenthesis
          ) {
          ...
          if (this.isNamedCombinator()) {
            ...
            this.position++;
          } else if (O[this.currToken[_.FIELDS.TYPE]]) {
          } else if (!u) {
            this.unexpected();
          }
          if (c) {
            ...
            if (v.endsWith(" ") && g.endsWith(" ")) {
              ...
              S.spaces.before = g.slice(0, g.length - 1);
            } else if (v.startsWith(" ") && g.startsWith(" ")) {
              b.after = v.slice(1);
              S.spaces.after = g.slice(1);
            } else {
              S.value = g;
            }
            c = new d["default"]({
        ...
        e.comma = function comma() {
          ...
          var e = new i["default"]({
            ...
            sourceIndex: this.tokens[this.position + 1][_.FIELDS.START_POS],
          });
        ...
        e.unexpectedPipe = function unexpectedPipe() {
          return this.error(
            "Unexpected '|'.",
            this.currToken[_.FIELDS.START_POS],
          );
        ...
        e.parentheses = function parentheses() {
          ...
          if (e && e.type === g.PSEUDO) {
            var r = new i["default"]({
              ...
              sourceIndex: this.tokens[this.position][_.FIELDS.START_POS],
            });
            var n = this.current;
            e.append(r);
            this.current = r;
        ...
        e.locateNextMeaningfulToken = function locateNextMeaningfulToken(e) {
          ...
          while (t < this.tokens.length) {
            if (w[this.tokens[t][_.FIELDS.TYPE]]) {
              t++;
              continue;
            } else {
              return t;
    ...
    128: (e, t, r) => {
      ...
      t.__esModule = true;
      t["default"] = void 0;
      var n = _interopRequireDefault(r(553));
      function _interopRequireDefault(e) {
        return e && e.__esModule ? e : { default: e };
      }
      var i = (function () {
    ...
    767: (e, t, r) => {
      ...
      var i = _interopRequireDefault(r(255));
      var s = _interopRequireDefault(r(929));
      var o = r(763);
      var a;
      function _interopRequireDefault(e) {
        return e && e.__esModule ? e : { default: e };
    ...
    242: (e, t, r) => {
      ...
      t.__esModule = true;
      t["default"] = void 0;
      var n = _interopRequireDefault(r(616));
      var i = r(763);
      function _interopRequireDefault(e) {
        return e && e.__esModule ? e : { default: e };
      }
    ...
    875: (e, t, r) => {
      ...
      t.universal =
        ...
        t.attribute =
          void 0;
      var n = _interopRequireDefault(r(767));
      var i = _interopRequireDefault(r(692));
      var s = _interopRequireDefault(r(242));
      var o = _interopRequireDefault(r(416));
      var a = _interopRequireDefault(r(114));
      var u = _interopRequireDefault(r(426));
      var c = _interopRequireDefault(r(330));
      var l = _interopRequireDefault(r(478));
      var f = _interopRequireDefault(r(193));
      var p = _interopRequireDefault(r(54));
      var d = _interopRequireDefault(r(830));
      var h = _interopRequireDefault(r(627));
      function _interopRequireDefault(e) {
        return e && e.__esModule ? e : { default: e };
      }
      ...
      t.pseudo = m;
      var O = function root(e) {
        return new l["default"](e);
      };
      t.root = O;
      var w = function selector(e) {
        return new f["default"](e);
      };
      t.selector = w;
      var P = function string(e) {
        return new p["default"](e);
      };
      t.string = P;
      var T = function tag(e) {
        return new d["default"](e);
      };
      t.tag = T;
      var k = function universal(e) {
    ...
    300: (e, t, r) => {
      ...
      t["default"] = void 0;
      var n = _interopRequireDefault(r(616));
      var i = _interopRequireWildcard(r(763));
      function _getRequireWildcardCache(e) {
        if (typeof WeakMap !== "function") return null;
        var t = new WeakMap();
        var r = new WeakMap();
    ...
    93: (e, t, r) => {
      "use strict";
      t.__esModule = true;
      t.isComment = t.isCombinator = t.isClassName = t.isAttribute = void 0;
      t.isContainer = isContainer;
      t.isIdentifier = void 0;
      ...
      t.isUniversal = t.isTag = t.isString = t.isSelector = t.isRoot = void 0;
      var n = r(763);
      var i;
      var s =
        ((i = {}),
    ...
    330: (e, t, r) => {
      ...
      t.__esModule = true;
      t["default"] = void 0;
      var n = _interopRequireDefault(r(300));
      var i = r(763);
      function _interopRequireDefault(e) {
        return e && e.__esModule ? e : { default: e };
      }
    ...
    830: (e, t, r) => {
      ...
      t.__esModule = true;
      t["default"] = void 0;
      var n = _interopRequireDefault(r(929));
      var i = r(763);
      function _interopRequireDefault(e) {
        return e && e.__esModule ? e : { default: e };
      }
    ...
    999: (e, t, r) => {
      ...
      t.FIELDS = void 0;
      t["default"] = tokenize;
      var n = _interopRequireWildcard(r(530));
      var i, s;
      function _getRequireWildcardCache(e) {
        if (typeof WeakMap !== "function") return null;
        var t = new WeakMap();
    ...
    362: (e, t, r) => {
      "use strict";
      t.__esModule = true;
      t.unesc = t.stripComments = t.getProp = t.ensureObject = void 0;
      var n = _interopRequireDefault(r(255));
      t.unesc = n["default"];
      var i = _interopRequireDefault(r(735));
      t.getProp = i["default"];
      var s = _interopRequireDefault(r(184));
      t.ensureObject = s["default"];
      var o = _interopRequireDefault(r(326));
